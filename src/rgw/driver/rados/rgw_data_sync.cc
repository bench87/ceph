...
static const string datalog_sync_status_shard_prefix =
    "datalog.sync-status.shard";
static const string datalog_sync_full_sync_index_prefix =
...
class RGWReadDataSyncStatusMarkersCR : public RGWShardCollectCR {
  ...
  int handle_result(int r) override {
    ...
    if (r < 0) {
      ldout(cct, 4) << "failed to read data sync status: " << cpp_strerror(r)
                    << dendl;
    }
    return r;
  }
...
public:
  RGWReadDataSyncStatusMarkersCR(RGWDataSyncCtx *sc, int num_shards,
      ...
      : RGWShardCollectCR(sc->cct, MAX_CONCURRENT_SHARDS), sc(sc), env(sc->env),
        num_shards(num_shards), markers(markers), objvs(objvs) {}
...
bool RGWReadDataSyncStatusMarkersCR::spawn_next() {
  ...
  spawn(new CR(env->dpp, env->driver,
               rgw_raw_obj(env->svc->zone->get_zone_params().log_pool,
...
public:
  RGWReadDataSyncRecoveringShardsCR(
      RGWDataSyncCtx *sc, uint64_t _max_entries, int _num_shards,
      std::vector<RGWRadosGetOmapKeysCR::ResultPtr> &omapkeys)
      : RGWShardCollectCR(sc->cct, MAX_CONCURRENT_SHARDS), sc(sc), env(sc->env),
...
bool RGWReadDataSyncRecoveringShardsCR::spawn_next() {
  ...
  string error_oid =
      RGWDataSyncStatusManager::shard_obj_name(sc->source_zone, shard_id) +
      ".retry";
  auto &shard_keys = omapkeys[shard_id];
  shard_keys = std::make_shared<RGWRadosGetOmapKeysCR::Result>();
  spawn(new RGWRadosGetOmapKeysCR(
            env->driver,
            rgw_raw_obj(env->svc->zone->get_zone_params().log_pool, error_oid),
            marker, max_entries, shard_keys),
        false);
...
public:
  RGWReadDataSyncStatusCoroutine(RGWDataSyncCtx *_sc,
      ...
      : RGWCoroutine(_sc->cct), sc(_sc), sync_env(sc->env),
        sync_status(_status), objv_tracker(objv_tracker), objvs(objvs) {}
...
public:
  RGWReadRemoteDataLogShardInfoCR(RGWDataSyncCtx *_sc, int _shard_id,
                                  RGWDataChangesLogInfo *_shard_info)
      : RGWCoroutine(_sc->cct), sc(_sc), sync_env(_sc->env), http_op(NULL),
        shard_id(_shard_id), shard_info(_shard_info) {}

  int operate(const DoutPrefixProvider *dpp) override {
    reenter(this) {
      static constexpr int NUM_ENPOINT_IOERROR_RETRIES = 20;
      for (tries = 0; tries < NUM_ENPOINT_IOERROR_RETRIES; tries++) {
        ldpp_dout(dpp, 20) << "read remote datalog shard info. shard_id="
                           << shard_id << " retries=" << tries << dendl;

        yield {
          char buf[16];
          snprintf(buf, sizeof(buf), "%d", shard_id);
          rgw_http_param_pair pairs[] = {
              {"type", "data"}, {"id", buf}, {"info", NULL}, {NULL, NULL}};

          string p = "/admin/log/";

          http_op = new RGWRESTReadResource(sc->conn, p, pairs, NULL,
          ...
          if (ret < 0) {
            ...
            log_error() << "failed to send http operation: "
                        << http_op->to_str() << " ret=" << ret << std::endl;
            http_op->put();
        ...
        if (op_ret < 0) {
          if (op_ret == -EIO && tries < NUM_ENPOINT_IOERROR_RETRIES - 1) {
            ldpp_dout(dpp, 20)
                << "failed to fetch remote datalog shard info. retry. shard_id="
                << shard_id << dendl;
            continue;
          } else {
...
public:
  ...
  int operate(const DoutPrefixProvider *dpp) override {
    reenter(this) {
      static constexpr int NUM_ENPOINT_IOERROR_RETRIES = 20;
      for (tries = 0; tries < NUM_ENPOINT_IOERROR_RETRIES; tries++) {
        ldpp_dout(dpp, 20) << "read remote datalog shard. shard_id=" << shard_id
                           << " retries=" << tries << dendl;
        ...
        if (op_ret < 0) {
          if (op_ret == -EIO && tries < NUM_ENPOINT_IOERROR_RETRIES - 1) {
            ldpp_dout(dpp, 20)
                << "failed to read remote datalog shard. retry. shard_id="
                << shard_id << dendl;
            continue;
          } else {
...
public:
  RGWReadRemoteDataLogInfoCR(RGWDataSyncCtx *_sc, int _num_shards,
                             map<int, RGWDataChangesLogInfo> *_datalog_info)
      : RGWShardCollectCR(_sc->cct, READ_DATALOG_MAX_CONCURRENT), sc(_sc),
        sync_env(_sc->env), num_shards(_num_shards),
...
bool RGWReadRemoteDataLogInfoCR::spawn_next() {
  ...
  spawn(new RGWReadRemoteDataLogShardInfoCR(sc, shard_id,
                                            &(*datalog_info)[shard_id]),
        false);
  shard_id++;
  return true;
}
...
public:
  RGWListRemoteDataLogShardCR(RGWDataSyncCtx *sc, int _shard_id,
      ...
      : RGWSimpleCoroutine(sc->cct, NUM_ENPOINT_IOERROR_RETRIES), sc(sc),
        sync_env(sc->env), http_op(NULL), shard_id(_shard_id), marker(_marker),
        max_entries(_max_entries), result(_result) {}
  ...
  int send_request(const DoutPrefixProvider *dpp) override {
    ...
    http_op =
        new RGWRESTReadResource(conn, p, pairs, NULL, sync_env->http_manager);
    init_new_io(http_op);

    int ret = http_op->aio_read(dpp);
    if (ret < 0) {
      ldpp_dout(dpp, 0) << "ERROR: failed to read from " << p << dendl;
      log_error() << "failed to send http operation: " << http_op->to_str()
                  << " ret=" << ret << std::endl;
      http_op->put();
      return ret;
    }
...
#define READ_DATALOG_MAX_CONCURRENT 10
  ...
  int handle_result(int r) override {
    ...
    if (r < 0) {
      ldout(cct, 4) << "failed to list remote datalog: " << cpp_strerror(r)
                    << dendl;
    }
    return r;
  }
...
public:
  RGWListRemoteDataLogCR(RGWDataSyncCtx *_sc, map<int, string> &_shards,
                         int _max_entries_per_shard,
                         map<int, rgw_datalog_shard_data> *_result)
      : RGWShardCollectCR(_sc->cct, READ_DATALOG_MAX_CONCURRENT), sc(_sc),
        sync_env(_sc->env), max_entries_per_shard(_max_entries_per_shard),
        result(_result) {
    shards.swap(_shards);
    iter = shards.begin();
  }
...
bool RGWListRemoteDataLogCR::spawn_next() {
  ...
  spawn(new RGWListRemoteDataLogShardCR(sc, iter->first, iter->second,
...
public:
  ...
  int operate(const DoutPrefixProvider *dpp) override {
    ...
    reenter(this) {
      ...
      if (retcode < 0) {
        tn->log(0, SSTR("ERROR: failed to write sync status info with "
                        << retcode));
        return set_cr_error(retcode);
      }
      ...
      yield {
        ...
        if (!conn) {
          tn->log(0, SSTR("ERROR: connection to zone " << sc->source_zone
                                                       << " does not exist!"));
          return set_cr_error(-EIO);
        }
        for (uint32_t i = 0; i < num_shards; i++) {
          spawn(new RGWReadRemoteDataLogShardInfoCR(sc, i, &shards_info[i]),
                true);
        }
      }
      ...
      yield {
        ...
        for (uint32_t i = 0; i < num_shards; i++) {
          ...
          const auto &oid =
              RGWDataSyncStatusManager::shard_obj_name(sc->source_zone, i);
          auto &objv = objvs[i];
          objv.generate_new_write_ver(cct);
          using WriteMarkerCR = RGWSimpleRadosWriteCR<rgw_data_sync_marker>;
          spawn(new WriteMarkerCR(dpp, sync_env->driver, rgw_raw_obj{pool, oid},
                                  marker, &objv),
                true);
        }
      }
...
RGWRemoteDataLog::RGWRemoteDataLog(const DoutPrefixProvider *dpp,
    ...
    : RGWCoroutinesManager(driver->ctx(),
                           driver->getRados()->get_cr_registry()),
      dpp(dpp), driver(driver), cct(driver->ctx()),
      cr_registry(driver->getRados()->get_cr_registry()),
      async_rados(async_rados), http_manager(driver->ctx(), completion_mgr),
      data_sync_cr(NULL), initialized(false) {}

int RGWRemoteDataLog::read_log_info(const DoutPrefixProvider *dpp,
                                    rgw_datalog_info *log_info) {
  rgw_http_param_pair pairs[] = {{"type", "data"}, {NULL, NULL}};

  int ret = sc.conn->get_json_resource(dpp, "/admin/log", pairs, null_yield,
                                       *log_info);
  if (ret < 0) {
  ...
  ldpp_dout(dpp, 20) << "remote datalog, num_shards=" << log_info->num_shards
                     << dendl;

  return 0;
}

int RGWRemoteDataLog::read_source_log_shards_info(
  ...
  return run(dpp, new RGWReadRemoteDataLogInfoCR(&sc, log_info.num_shards,
                                                 shards_info));
}

int RGWRemoteDataLog::read_source_log_shards_next(
    const DoutPrefixProvider *dpp, map<int, string> shard_markers,
    map<int, rgw_datalog_shard_data> *result) {
  return run(dpp, new RGWListRemoteDataLogCR(&sc, shard_markers, 1, result));
}

int RGWRemoteDataLog::init(const rgw_zone_id &_source_zone, RGWRESTConn *_conn,
                           RGWSyncErrorLogger *_error_logger,
                           RGWSyncTraceManager *_sync_tracer,
                           RGWSyncModuleInstanceRef &_sync_module,
                           PerfCounters *counters) {
  sync_env.init(dpp, cct, driver, driver->svc(), async_rados, &http_manager,
                _error_logger, _sync_tracer, _sync_module, counters);
...
void RGWRemoteDataLog::finish() { stop(); }

int RGWRemoteDataLog::read_sync_status(const DoutPrefixProvider *dpp,
                                       rgw_data_sync_status *sync_status) {
...
int RGWRemoteDataLog::read_recovering_shards(const DoutPrefixProvider *dpp,
                                             const int num_shards,
  ...
  ret = crs.run(dpp, new RGWReadDataSyncRecoveringShardsCR(
                         &sc_local, max_entries, num_shards, omapkeys));
  http_manager.stop();
...
int RGWRemoteDataLog::init_sync_status(const DoutPrefixProvider *dpp,
  ...
  sc_local.env = &sync_env_local;
  ret = crs.run(dpp,
                new RGWRDL::DataSyncInitCR(&sc_local, num_shards, instance_id,
                                           tn, &sync_status, objvs));
  http_manager.stop();
  return ret;
}

static string full_data_sync_index_shard_oid(const rgw_zone_id &source_zone,
                                             int shard_id) {
  char buf[datalog_sync_full_sync_index_prefix.size() + 1 +
           source_zone.id.size() + 1 + 16];
  snprintf(buf, sizeof(buf), "%s.%s.%d",
           datalog_sync_full_sync_index_prefix.c_str(), source_zone.id.c_str(),
           shard_id);
  return string(buf);
}
  ...
  int operate(const DoutPrefixProvider *dpp) override {
    reenter(this) {
      yield {
        ...
        string p = "/admin/log/";
        call(new RGWReadRESTResourceCR<rgw_bucket_index_marker_info>(
            sync_env->cct, sc->conn, sync_env->http_manager, p, pairs, info));
      }
      if (retcode < 0) {
        return set_cr_error(retcode);
      }
...
public:
  RGWListBucketIndexesCR(RGWDataSyncCtx *sc, rgw_data_sync_status *sync_status,
                         std::vector<RGWObjVersionTracker> &objvs)
  ...
  int operate(const DoutPrefixProvider *dpp) override {
    reenter(this) {
      entries_index = std::make_unique<RGWShardedOmapCRManager>(
          ...
          sync_env->svc->zone->get_zone_params().log_pool, oid_prefix);
      ...
      do {
        yield {
          ...
          call(new RGWReadRESTResourceCR<read_metadata_list>(
              sync_env->cct, sc->conn, sync_env->http_manager, entrypoint,
              pairs, &result));
        }
        if (retcode < 0) {
          ldpp_dout(dpp, 0)
        ...
        for (iter = result.keys.begin(); iter != result.keys.end(); ++iter) {
          ldpp_dout(dpp, 20)
              << "list metadata: section=bucket.instance key=" << *iter
              << dendl;
          key = *iter;
          ...
          if (retcode < 0) {
            ldpp_dout(dpp, 0)
                << "ERROR: failed to fetch metadata for key: " << key << dendl;
            return set_cr_error(retcode);
          }
      ...
      if (!failed) {
        ...
        yield call(sync_env->error_logger->log_error_cr(
            dpp, sc->conn->get_remote_id(), "data.init", "", EIO,
            string("failed to build bucket instances map")));
      }
      while (collect(&ret, NULL)) {
        if (ret < 0) {
          yield call(sync_env->error_logger->log_error_cr(
              dpp, sc->conn->get_remote_id(), "data.init", "", -ret,
              string("failed to driver sync status: ") + cpp_strerror(-ret)));
          req_ret = ret;
        }
        yield;
      }
...
class RGWDataSyncShardMarkerTrack
    : public RGWSyncShardMarkerTrack<string, string> {
  RGWDataSyncCtx *sc;
  RGWDataSyncEnv *sync_env;
  string marker_oid;
...
public:
  RGWDataSyncShardMarkerTrack(RGWDataSyncCtx *_sc, const string &_marker_oid,
                              const rgw_data_sync_marker &_marker,
                              RGWSyncTraceNodeRef &_tn,
                              RGWObjVersionTracker &objv)
      : RGWSyncShardMarkerTrack(DATA_SYNC_UPDATE_MARKER_WINDOW), sc(_sc),
        sync_env(_sc->env), marker_oid(_marker_oid), sync_marker(_marker),
        tn(_tn), objv(objv) {}

  RGWCoroutine *store_marker(const string &new_marker, uint64_t index_pos,
                             const real_time &timestamp) override {
    sync_marker.marker = new_marker;
    sync_marker.pos = index_pos;
    sync_marker.timestamp = timestamp;

    tn->log(20, SSTR("updating marker marker_oid=" << marker_oid << " marker="
                                                   << new_marker));

    return new RGWSimpleRadosWriteCR<rgw_data_sync_marker>(
...
public:
  ...
  rgw_sync_pipe_info_entity(const rgw_sync_bucket_entity &e,
    ...
    if (!binfo || binfo->bucket_info.bucket != *e.bucket) {
  ...
  void update_empty_bucket_info(
      const std::map<rgw_bucket, all_bucket_info> &buckets_info) {
    if (_has_bucket_info) {
      return;
    }
  ...
  bool has_bucket_info() const { return _has_bucket_info; }
  ...
  const rgw_bucket &get_bucket() const { return bucket_info.bucket; }
...
std::ostream &operator<<(std::ostream &out,
                         const rgw_sync_pipe_info_entity &e) {
...
struct rgw_sync_pipe_handler_info {
  ...
  rgw_sync_pipe_handler_info(
      ...
      : handler(_handler), source(handler.source, source_bucket_info),
...
std::ostream &operator<<(std::ostream &out,
                         const rgw_sync_pipe_handler_info &p) {
...
struct rgw_sync_pipe_info_set {
  ...
  void insert(const RGWBucketSyncFlowManager::pipe_handler &handler,
              std::optional<all_bucket_info> &source_bucket_info,
              std::optional<all_bucket_info> &target_bucket_info) {
    rgw_sync_pipe_handler_info p(handler, source_bucket_info,
                                 target_bucket_info);
    handlers.insert(p);
  }
...
public:
  RGWRunBucketSourcesSyncCR(
      RGWDataSyncCtx *_sc,
      boost::intrusive_ptr<const RGWContinuousLeaseCR> lease_cr,
      const rgw_bucket_shard &source_bs, const RGWSyncTraceNodeRef &_tn_parent,
      std::optional<uint64_t> gen, ceph::real_time *progress);
...
public:
  RGWDataSyncSingleEntryCR(
      RGWDataSyncCtx *_sc, rgw::bucket_sync::Handle state,
    ...
    set_description() << "data sync single entry (source_zone="
                      << sc->source_zone << ") " << obligation;
    tn = sync_env->sync_tracer->add_node(
        _tn_parent, "entry", to_string(obligation.bs, obligation.gen));
  }
    ...
    reenter(this) {
      if (state->obligation) {
        ...
        if (state->obligation->timestamp < obligation.timestamp) {
          ...
          tn->log(10,
                  SSTR("canceling existing obligation " << *state->obligation));
        ...
        while ((state->obligation->timestamp == ceph::real_time() ||
          ...
          ldout(cct, 4) << "starting sync on "
                        << bucket_shard_str{state->key.first} << ' '
                        << *state->obligation << " progress timestamp "
                        << state->progress_timestamp << " progress " << progress
                        << dendl;
          yield call(
              new RGWRunBucketSourcesSyncCR(sc, lease_cr, state->key.first, tn,
                                            state->obligation->gen, &progress));
          if (retcode < 0) {
            break;
          }
          state->progress_timestamp =
              std::max(progress, state->progress_timestamp);
        }
        // any new obligations will process themselves
        complete = std::move(*state->obligation);
        state->obligation.reset();

        tn->log(10, SSTR("sync finished on "
                         << bucket_shard_str{state->key.first} << " progress="
                         << progress << ' ' << complete << " r=" << retcode));
      }
      sync_status = retcode;
      ...
      if (sync_status == -ENOENT) {
        ...
        // error_repo forever
        tn->log(0, SSTR("WARNING: skipping data log entry for missing bucket "
                        << complete->bs));
        sync_status = 0;
      }
      ...
      if (sync_status < 0) {
        ...
        if (sync_status != -EBUSY && sync_status != -EAGAIN) {
          yield call(sync_env->error_logger->log_error_cr(
              dpp, sc->conn->get_remote_id(), "data",
              to_string(complete->bs, complete->gen), -sync_status,
              string("failed to sync bucket instance: ") +
                  cpp_strerror(-sync_status)));
          if (retcode < 0) {
            tn->log(0, SSTR("ERROR: failed to log sync failure: retcode="
                            << retcode));
          }
        }
        if (complete->timestamp != ceph::real_time{}) {
          tn->log(10,
                  SSTR("writing " << *complete << " to error repo for retry"));
          yield call(rgw::error_repo::write_cr(
          ...
          if (retcode < 0) {
            tn->log(
                0,
                SSTR("ERROR: failed to log sync failure in error repo: retcode="
                     << retcode));
          }
        }
      } else if (complete->retry) {
        yield call(rgw::error_repo::remove_cr(
            sync_env->driver->getRados()->get_rados_handle(), error_repo,
            rgw::error_repo::encode_key(complete->bs, complete->gen),
            complete->timestamp));
        if (retcode < 0) {
...
rgw_raw_obj datalog_oid_for_error_repo(RGWDataSyncCtx *sc,
                                       rgw::sal::RadosStore *driver,
                                       rgw_pool &pool, rgw_bucket_shard &bs) {
  int datalog_shard = driver->svc()->datalog_rados->choose_oid(bs);
  string oid =
      RGWDataSyncStatusManager::shard_obj_name(sc->source_zone, datalog_shard);
...
public:
  RGWDataIncrementalSyncFullObligationCR(RGWDataSyncCtx *_sc,
                                         rgw_bucket_shard &_source_bs,
                                         const rgw_raw_obj &error_repo,
                                         const std::string &_error_marker,
                                         ceph::real_time &_timestamp,
                                         RGWSyncTraceNodeRef &_tn)
      : RGWCoroutine(_sc->cct), sc(_sc), sync_env(_sc->env),
        source_bs(_source_bs), error_repo(error_repo),
        error_marker(_error_marker), timestamp(_timestamp),
        tn(sync_env->sync_tracer->add_node(_tn, "error_repo",
                                           SSTR(bucket_shard_str(source_bs)))) {
  }

  int operate(const DoutPrefixProvider *dpp) override {
    reenter(this) {
      yield call(new RGWReadRemoteBucketIndexLogInfoCR(sc, source_bs.bucket,
                                                       &remote_info));
      if (retcode == -ENOENT) {
        // don't retry if bucket instance does not exist
        tn->log(10, SSTR("bucket instance or log layout does not exist on "
                         "source for bucket "
                         << source_bs.bucket));
        yield call(rgw::error_repo::remove_cr(
            sync_env->driver->getRados()->get_rados_handle(), error_repo,
      ...
      for (; each != remote_info.generations.cend(); each++) {
        for (sid = 0; sid < each->num_shards; sid++) {
          ...
          error_repo =
              datalog_oid_for_error_repo(sc, sync_env->driver, pool, source_bs);
          tn->log(10, SSTR("writing shard_id " << sid << " of gen " << each->gen
                                               << " to error repo for retry"));
          yield_spawn_window(
              rgw::error_repo::write_cr(
                  sync_env->driver->getRados()->get_rados_handle(), error_repo,
                  rgw::error_repo::encode_key(bs, each->gen), timestamp),
      ...
      // once everything succeeds, remove the full sync obligation from the
      // error repo
      yield call(rgw::error_repo::remove_cr(
          sync_env->driver->getRados()->get_rados_handle(), error_repo,
          error_marker, timestamp));
...
RGWCoroutine *data_sync_single_entry(
    RGWDataSyncCtx *sc, const rgw_bucket_shard &src,
    std::optional<uint64_t> gen, const std::string marker,
    ceph::real_time timestamp,
    boost::intrusive_ptr<const RGWContinuousLeaseCR> lease_cr,
    boost::intrusive_ptr<rgw::bucket_sync::Cache> bucket_shard_cache,
    RGWDataSyncShardMarkerTrack *marker_tracker, rgw_raw_obj error_repo,
    RGWSyncTraceNodeRef &tn, bool retry) {
  auto state = bucket_shard_cache->get(src, gen);
  auto obligation =
      rgw_data_sync_obligation{src, gen, marker, timestamp, retry};
  return new RGWDataSyncSingleEntryCR(sc, std::move(state),
                                      std::move(obligation), &*marker_tracker,
                                      error_repo, lease_cr.get(), tn);
}

static ceph::real_time
timestamp_for_bucket_shard(rgw::sal::RadosStore *driver,
...
public:
  RGWDataFullSyncSingleEntryCR(
      RGWDataSyncCtx *_sc, const rgw_pool &_pool,
      const rgw_bucket_shard &_source_bs, const std::string &_key,
      const rgw_data_sync_status &sync_status, const rgw_raw_obj &_error_repo,
      ceph::real_time _timestamp,
      boost::intrusive_ptr<const RGWContinuousLeaseCR> _lease_cr,
      boost::intrusive_ptr<rgw::bucket_sync::Cache> _bucket_shard_cache,
      RGWDataSyncShardMarkerTrack *_marker_tracker, RGWSyncTraceNodeRef &_tn)
      : RGWCoroutine(_sc->cct), sc(_sc), sync_env(_sc->env), pool(_pool),
        source_bs(_source_bs), key(_key), error_repo(_error_repo),
        timestamp(_timestamp), lease_cr(std::move(_lease_cr)),
        bucket_shard_cache(_bucket_shard_cache),
        marker_tracker(_marker_tracker), tn(_tn) {
    error_inject =
        (sync_env->cct->_conf->rgw_sync_data_full_inject_err_probability > 0);
  }
  ...
  int operate(const DoutPrefixProvider *dpp) override {
    reenter(this) {
      if (error_inject &&
          rand() % 10000 <
              cct->_conf->rgw_sync_data_full_inject_err_probability * 10000.0) {
      ...
      if (retcode < 0) {
        tn->log(10,
                SSTR("full sync: failed to read remote bucket info. Writing "
                     << source_bs.shard_id << " to error repo for retry"));
        yield call(rgw::error_repo::write_cr(
            sync_env->driver->getRados()->get_rados_handle(), error_repo,
            rgw::error_repo::encode_key(source_bs, std::nullopt), timestamp));
        if (retcode < 0) {
          tn->log(0, SSTR("ERROR: failed to log "
                          << source_bs.shard_id
      ...
      // wait to sync the first shard of the oldest generation and then sync all
      // other shards. if any of the operations fail at any time, write them
      // into error repo for later retry.

      each = remote_info.generations.begin();
      for (; each != remote_info.generations.end(); each++) {
        for (sid = 0; sid < each->num_shards; sid++) {
          source_bs.shard_id = sid;
          // use the error repo and sync status timestamp from the datalog shard
          // corresponding to source_bs
          error_repo =
              datalog_oid_for_error_repo(sc, sync_env->driver, pool, source_bs);
          timestamp = timestamp_for_bucket_shard(sync_env->driver, sync_status,
                                                 source_bs);
          if (retcode < 0) {
            tn->log(10, SSTR("Write " << source_bs.shard_id
                                      << " to error repo for retry"));
            yield_spawn_window(
                rgw::error_repo::write_cr(
            ...
            shard_cr = data_sync_single_entry(
                sc, source_bs, each->gen, key, timestamp, lease_cr,
                bucket_shard_cache, nullptr, error_repo, tn, false);
            tn->log(10, SSTR("full sync: syncing shard_id " << sid << " of gen "
                                                            << each->gen));
            if (first_shard) {
              first_shard = false;
              yield call(shard_cr);
...
protected:
  ...
  int parse_bucket_key(const std::string &key, rgw_bucket_shard &bs) const {
    int ret = rgw_bucket_parse_bucket_key(sc->env->cct, key, &bs.bucket,
                                          &bs.shard_id);
    // for the case of num_shards 0, shard_id gets a value of -1
    // because of the way bucket instance gets parsed in the absence of shard_id
    // delimiter. interpret it as a non-negative value.
    if (ret == 0) {
      if (bs.shard_id < 0) {
        bs.shard_id = 0;
      }
...
public:
  RGWDataFullSyncShardCR(
      ...
      const rgw_data_sync_status &sync_status, RGWObjVersionTracker &objv,
      const boost::intrusive_ptr<rgw::bucket_sync::Cache> &bucket_shard_cache)
      : RGWDataBaseSyncShardCR(sc, pool, shard_id, sync_marker, tn, status_oid,
                               error_repo, std::move(lease_cr), sync_status,
  ...
  int operate(const DoutPrefixProvider *dpp) override {
    reenter(this) {
      ...
      do {
        ...
        yield call(new RGWRadosGetOmapValsCR(
            sc->env->driver, rgw_raw_obj(pool, oid), sync_marker.marker,
        ...
        for (; iter != entries.end(); ++iter) {
          ...
          if (!marker_tracker->start(iter->first, total_entries,
                                     entry_timestamp)) {
            tn->log(0, SSTR("ERROR: cannot start syncing "
                            << iter->first << ". Duplicate entry?"));
          } else {
            tn->log(10, SSTR("timestamp for " << iter->first
                                              << " is :" << entry_timestamp));
            yield_spawn_window(
                new RGWDataFullSyncSingleEntryCR(
                    sc, pool, source_bs, iter->first, sync_status, error_repo,
                    entry_timestamp, lease_cr, bucket_shard_cache,
                    &*marker_tracker, tn),
                sc->lcc.adj_concurrency(cct->_conf->rgw_data_sync_spawn_window),
                std::nullopt);
          }
      ...
      if (lost_bid) {
        ...
        yield call(new RGWSimpleRadosWriteCR<rgw_data_sync_marker>(
            sc->env->dpp, sc->env->driver, rgw_raw_obj(pool, status_oid),
            sync_marker, &objv));
        if (retcode < 0) {
          tn->log(
              0, SSTR("ERROR: failed to set sync marker: retcode=" << retcode));
          return set_cr_error(retcode);
        }
...
class RGWDataIncSyncShardCR : public RGWDataBaseSyncShardCR {
  ...
  utime_t get_idle_interval() const {
    ceph::timespan interval =
        std::chrono::seconds(cct->_conf->rgw_data_sync_poll_interval);
    ...
    reenter(this) {
      ...
      do {
        ...
        for (modified_iter = current_modified.begin();
             modified_iter != current_modified.end(); ++modified_iter) {
          if (!lease_cr->is_locked()) {
            tn->log(1, "lease is lost, abort");
            lost_lock = true;
            break;
        ...
        if (error_retry_time <= ceph::coarse_real_clock::now()) {
          ...
          for (; iter != error_entries.end(); ++iter) {
            ...
            if (retcode < 0) {
              tn->log(1,
                      SSTR("failed to parse bucket shard: " << error_marker));
              spawn(rgw::error_repo::remove_cr(
                        sc->env->driver->getRados()->get_rados_handle(),
                        error_repo, error_marker, entry_timestamp),
                    false);
              continue;
            }
            ...
            if (!gen) {
              ...
              spawn(new RGWDataIncrementalSyncFullObligationCR(
                        sc, source_bs, error_repo, error_marker,
                        entry_timestamp, tn),
                    false);
            } else {
        ...
        yield call(new RGWReadRemoteDataLogShardCR(
            sc, shard_id, sync_marker.marker, &next_marker, &log_entries,
            &truncated));
        ...
        for (log_iter = log_entries.begin(); log_iter != log_entries.end();
             ++log_iter) {
          if (!lease_cr->is_locked()) {
            tn->log(1, "lease is lost, abort");
            lost_lock = true;
          ...
          if (!marker_tracker->start(log_iter->log_id, 0,
                                     log_iter->log_timestamp)) {
            tn->log(0, SSTR("ERROR: cannot start syncing "
                            << log_iter->log_id << ". Duplicate entry?"));
          } else {
            tn->log(1, SSTR("incremental sync on "
                            << log_iter->entry.key << "shard: " << shard_id
                            << "on gen " << log_iter->entry.gen));
            yield_spawn_window(
                data_sync_single_entry(
                    sc, source_bs, log_iter->entry.gen, log_iter->log_id,
                    log_iter->log_timestamp, lease_cr, bucket_shard_cache,
                    &*marker_tracker, error_repo, tn, false),
                sc->lcc.adj_concurrency(cct->_conf->rgw_data_sync_spawn_window),
                [&](uint64_t stack_id, int ret) {
                  if (ret < 0) {
                    tn->log(10, SSTR("data_sync_single_entry returned error: "
                                     << ret));
                    cbret = ret;
                  }
                  return 0;
...
public:
  RGWDataSyncShardCR(RGWDataSyncCtx *const _sc, const rgw_pool &pool,
                     ...
                     const rgw_data_sync_status &sync_status,
                     RGWSyncTraceNodeRef &tn, RGWObjVersionTracker &objv,
                     bool *reset_backoff)
      : RGWCoroutine(_sc->cct), sc(_sc), pool(pool), shard_id(shard_id),
        sync_marker(marker), sync_status(sync_status), tn(tn), objv(objv),
  ...
  int operate(const DoutPrefixProvider *dpp) override {
    reenter(this) {
      ...
      *reset_backoff = true;
      ...
      objv.clear();
      yield call(new RGWSimpleRadosReadCR<rgw_data_sync_marker>(
          sync_env->dpp, sync_env->driver, rgw_raw_obj(pool, status_oid),
      ...
      while (true) {
        if (sync_marker.state == rgw_data_sync_marker::FullSync) {
          yield call(new RGWDataFullSyncShardCR(
              sc, pool, shard_id, sync_marker, tn, status_oid, error_repo,
              lease_cr, sync_status, objv, bucket_shard_cache));
          if (retcode < 0) {
            if (retcode != -EBUSY) {
          ...
          yield call(new RGWDataIncSyncShardCR(
              sc, pool, shard_id, sync_marker, tn, status_oid, error_repo,
              lease_cr, sync_status, objv, bucket_shard_cache, inc_lock,
              modified_shards));
          if (retcode < 0) {
            if (retcode != -EBUSY) {
  ...
  void init_lease_cr() {
    ...
    lease_cr.reset(new RGWContinuousLeaseCR(
        sync_env->async_rados, driver, rgw_raw_obj(pool, status_oid), lock_name,
        lock_duration, this, &sc->lcc));
...
public:
  RGWDataSyncShardControlCR(RGWDataSyncCtx *_sc, const rgw_pool &_pool,
      ...
      : RGWBackoffControlCR(_sc->cct, false), sc(_sc), sync_env(_sc->env),
        pool(_pool), shard_id(_shard_id), sync_marker(_marker), objv(objv) {
    tn = sync_env->sync_tracer->add_node(_tn_parent, "shard",
                                         std::to_string(shard_id));
  }

  RGWCoroutine *alloc_cr() override {
    return new RGWDataSyncShardCR(sc, pool, shard_id, sync_marker, sync_status,
                                  tn, objv, backoff_ptr());
  }

  RGWCoroutine *alloc_finisher_cr() override {
    return new RGWSimpleRadosReadCR<rgw_data_sync_marker>(
        sync_env->dpp, sync_env->driver,
        rgw_raw_obj(sync_env->svc->zone->get_zone_params().log_pool,
                    RGWDataSyncStatusManager::shard_obj_name(sc->source_zone,
                                                             shard_id)),
        &sync_marker, true, &objv);
  }
...
public:
  RGWDataSyncShardNotifyCR(RGWDataSyncEnv *_sync_env, RGWSyncTraceNodeRef &_tn)
      : RGWCoroutine(_sync_env->cct), sync_env(_sync_env), tn(_tn) {}
...
public:
  RGWDataSyncCR(RGWDataSyncCtx *_sc, uint32_t _num_shards,
                RGWSyncTraceNodeRef &_tn, bool *_reset_backoff)
      : RGWCoroutine(_sc->cct), sc(_sc), sync_env(_sc->env),
        num_shards(_num_shards), reset_backoff(_reset_backoff), tn(_tn) {}
  ...
  int operate(const DoutPrefixProvider *dpp) override {
    reenter(this) {
      ...
      yield {
        ...
        notify_stack.reset(
            spawn(new RGWDataSyncShardNotifyCR(sync_env, tn), false));
      }
      ...
      if (retcode < 0 && retcode != -ENOENT) {
        tn->log(
            0, SSTR("ERROR: failed to fetch sync status, retcode=" << retcode));
        return set_cr_error(retcode);
      }
      ...
      if ((rgw_data_sync_info::SyncState)sync_status.sync_info.state !=
        ...
        yield call(new RGWReadDataSyncStatusCoroutine(sc, &sync_status,
                                                      &obj_version, objvs));
        if (retcode < 0) {
          tn->log(0, SSTR("ERROR: failed to fetch sync status, retcode="
                          << retcode));
          return set_cr_error(retcode);
        }
      }
      ...
      if ((rgw_data_sync_info::SyncState)sync_status.sync_info.state ==
        ...
        yield call(new RGWInitDataSyncStatusCoroutine(
            sc, num_shards, instance_id, tn, &sync_status, init_lease,
            obj_version, objvs));
        if (retcode < 0) {
      ...
      if ((rgw_data_sync_info::SyncState)sync_status.sync_info.state ==
        ...
        if (retcode < 0) {
          tn->log(0, SSTR("ERROR: sync module init_sync() failed, retcode="
                          << retcode));
          return set_cr_error(retcode);
        }
        ...
        if (retcode < 0) {
          tn->log(0, SSTR("ERROR: failed to build full sync maps, retcode="
                          << retcode));
          return set_cr_error(retcode);
        }
        ...
        if (retcode < 0) {
          tn->log(0, SSTR("ERROR: failed to write sync status, retcode="
                          << retcode));
          return set_cr_error(retcode);
        }
      ...
      if ((rgw_data_sync_info::SyncState)sync_status.sync_info.state ==
        ...
        yield {
          ...
          for (map<uint32_t, rgw_data_sync_marker>::iterator iter =
                   sync_status.sync_markers.begin();
               iter != sync_status.sync_markers.end(); ++iter) {
            RGWDataSyncShardControlCR *cr = new RGWDataSyncShardControlCR(
                sc, sync_env->svc->zone->get_zone_params().log_pool,
  ...
  RGWCoroutine *set_sync_info_cr() {
    return new RGWSimpleRadosWriteCR<rgw_data_sync_info>(
        sync_env->dpp, sync_env->driver,
        rgw_raw_obj(sync_env->svc->zone->get_zone_params().log_pool,
                    RGWDataSyncStatusManager::sync_status_oid(sc->source_zone)),
  ...
  void wakeup(int shard_id, bc::flat_set<rgw_data_notify_entry> &entries) {
    ...
    map<int, RGWDataSyncShardControlCR *>::iterator iter =
        shard_crs.find(shard_id);
...
public:
  ...
  RGWCoroutine *remove_object(const DoutPrefixProvider *dpp, RGWDataSyncCtx *sc,
                              rgw_bucket_sync_pipe &sync_pipe, rgw_obj_key &key,
                              real_time &mtime, bool versioned,
...
public:
  ...
  RGWDataSyncModule *get_data_handler() override { return &data_handler; }
  bool supports_user_writes() override { return true; }
};

int RGWDefaultSyncModule::create_instance(const DoutPrefixProvider *dpp,
                                          CephContext *cct,
...
class RGWUserPermHandler {
  ...
  struct Init : public RGWGenericAsyncCR::Action {
    ...
    Init(RGWUserPermHandler *handler)
        : sync_env(handler->sync_env), uid(handler->uid), info(handler->info) {}
...
public:
  RGWUserPermHandler(RGWDataSyncEnv *_sync_env, const rgw_user &_uid)
      : sync_env(_sync_env), uid(_uid) {}

  RGWCoroutine *init_cr() {
    info = make_shared<_info>();
    init_action = make_shared<Init>(this);

    return new RGWGenericAsyncCR(sync_env->cct, sync_env->async_rados,
                                 init_action);
  }
  ...
  public:
    ...
    int init(RGWUserPermHandler *handler, const RGWBucketInfo &bucket_info,
  ...
  static int policy_from_attrs(CephContext *cct,
    ...
    try {
      ...
      ldout(cct, 0) << "ERROR: " << __func__
                    << "(): could not decode policy, caught buffer::error"
                    << dendl;
      return -EIO;
    }
  ...
  int init_bucket(const RGWBucketInfo &bucket_info,
                  const map<string, bufferlist> &bucket_attrs, Bucket *bs) {
    return bs->init(this, bucket_info, bucket_attrs);
  }
};

int RGWUserPermHandler::Bucket::init(
    RGWUserPermHandler *handler, const RGWBucketInfo &bucket_info,
    const map<string, bufferlist> &bucket_attrs) {
  sync_env = handler->sync_env;
  info = handler->info;

  int r = RGWUserPermHandler::policy_from_attrs(sync_env->cct, bucket_attrs,
                                                &bucket_acl);
  if (r < 0) {
    return r;
  }

  ps.emplace(sync_env->cct, info->env, info->identity.get(), bucket_info,
             info->identity->get_perm_mask(), false, /* defer to bucket acls */
             nullptr,                                /* referer */
             false);                                 /* request_payer */
...
bool RGWUserPermHandler::Bucket::verify_bucket_permission(int perm) {
  return verify_bucket_permission_no_policy(sync_env->dpp, &(*ps),
                                            info->user_acl, bucket_acl, perm);
}

bool RGWUserPermHandler::Bucket::verify_object_permission(
    const map<string, bufferlist> &obj_attrs, int perm) {
  ...
  return verify_bucket_permission_no_policy(sync_env->dpp, &(*ps), bucket_acl,
                                            obj_acl, perm);
}
...
public:
  RGWFetchObjFilter_Sync(
      ...
      std::shared_ptr<bool> &_need_retry)
      : sync_pipe(_sync_pipe), bucket_perms(_bucket_perms),
        verify_dest_params(std::move(_verify_dest_params)),
        need_retry(_need_retry) {
    *need_retry = false;
  }

  int filter(CephContext *cct, const rgw_obj_key &source_key,
...
int RGWFetchObjFilter_Sync::filter(
    CephContext *cct, const rgw_obj_key &source_key,
    const RGWBucketInfo &dest_bucket_info,
    std::optional<rgw_placement_rule> dest_placement_rule,
    const map<string, bufferlist> &obj_attrs,
    std::optional<rgw_user> *poverride_owner,
  ...
  if (!sync_pipe.info.handler.find_obj_params(source_key, obj_tags.get_tags(),
                                              &params)) {
    return abort_err;
  }

  if (verify_dest_params && !(*verify_dest_params == params.dest)) {
    /* raced! original dest params were different, will need to retry */
    ldout(cct, 0) << "WARNING: " << __func__
                  << ": pipe dest params are different than original params, "
                     "must have raced with object rewrite, retrying"
                  << dendl;
    *need_retry = true;
    return -ECANCELED;
  }
  ...
  if (params.dest.acl_translation) {
    ...
    if (!acl_translation_owner.empty()) {
      if (params.mode == rgw_sync_pipe_params::MODE_USER &&
        ...
        ldout(cct, 0) << "ERROR: " << __func__
                      << ": acl translation was requested, but user ("
                      << acl_translation_owner << ") is not dest bucket owner ("
                      << dest_bucket_info.owner << ")" << dendl;
        return -EPERM;
      }
  ...
  if (params.mode == rgw_sync_pipe_params::MODE_USER) {
    if (!bucket_perms->verify_object_permission(obj_attrs, RGW_PERM_READ)) {
      ldout(cct, 0)
          << "ERROR: " << __func__
          << ": permission check failed: user not allowed to fetch object"
          << dendl;
      return -EPERM;
    }
  }

  if (!dest_placement_rule && params.dest.storage_class) {
  ...
  return RGWFetchObjFilter_Default::filter(cct, source_key, dest_bucket_info,
                                           dest_placement_rule, obj_attrs,
                                           poverride_owner, prule);
}
...
public:
  RGWObjFetchCR(RGWDataSyncCtx *_sc, rgw_bucket_sync_pipe &_sync_pipe,
                rgw_obj_key &_key, std::optional<rgw_obj_key> _dest_key,
                std::optional<uint64_t> _versioned_epoch, bool _stat_follow_olh,
                const rgw_zone_set_entry &source_trace_entry,
                rgw_zone_set *_zones_trace)
      : RGWCoroutine(_sc->cct), sc(_sc), sync_env(_sc->env),
        sync_pipe(_sync_pipe), key(_key), dest_key(_dest_key),
        versioned_epoch(_versioned_epoch), stat_follow_olh(_stat_follow_olh),
        source_trace_entry(source_trace_entry), zones_trace(_zones_trace) {}
...
#define MAX_RACE_RETRIES_OBJ_FETCH 10
      for (try_num = 0; try_num < MAX_RACE_RETRIES_OBJ_FETCH; ++try_num) {

        {
          ...
          if (!sync_pipe.info.handler.find_basic_info_without_tags(
                  key, &param_user, &param_acl_translation,
                  &param_storage_class, &param_mode, &need_more_info)) {
            if (!need_more_info) {
        ...
        if (need_more_info) {
          ldout(cct, 20) << "Could not determine exact policy rule for obj="
                         << key << ", will read source object attributes"
                         << dendl;
          /*
           * we need to fetch info about source object, so that we can determine
           * the correct policy configuration. This can happen if there are
           * multiple policy rules, and some depend on the object tagging */
          yield call(new RGWStatRemoteObjCR(
              sync_env->async_rados, sync_env->driver, sc->source_zone,
              sync_pipe.info.source_bs.bucket, key, &src_mtime, &src_size,
              &src_etag, &src_attrs, &src_headers));
          if (retcode < 0) {
            return set_cr_error(retcode);
          }
          ...
          rgw_sync_pipe_params params;
          if (!sync_pipe.info.handler.find_obj_params(key, obj_tags.get_tags(),
                                                      &params)) {
            return set_cr_error(-ERR_PRECONDITION_FAILED);
          }
        ...
        if (param_mode == rgw_sync_pipe_params::MODE_USER) {
          if (!param_user) {
            ldout(cct, 20) << "ERROR: " << __func__
                           << ": user level sync but user param not set"
                           << dendl;
          ...
          if (retcode < 0) {
            ldout(cct, 20) << "ERROR: " << __func__
                           << ": failed to init user perms manager for uid="
                           << *param_user << dendl;
            return set_cr_error(retcode);
          }
          ...
          if (r < 0) {
            ldout(cct, 20) << "ERROR: " << __func__
                           << ": failed to init bucket perms manager for uid="
                           << *param_user << " bucket="
                           << sync_pipe.source_bucket_info.bucket.get_key()
                           << dendl;
            return set_cr_error(retcode);
          }
          ...
          if (!dest_bucket_perms.verify_bucket_permission(RGW_PERM_WRITE)) {
            ldout(cct, 0) << "ERROR: " << __func__
                          << ": permission check failed: user not allowed to "
                             "write into bucket (bucket="
                          << sync_pipe.info.dest_bucket.get_key() << ")"
                          << dendl;
            return -EPERM;
          }
        ...
        yield {
          ...
          auto filter = make_shared<RGWFetchObjFilter_Sync>(
              sync_pipe, source_bucket_perms, std::move(dest_params),
              need_retry);

          call(new RGWFetchRemoteObjCR(
              sync_env->async_rados, sync_env->driver, sc->source_zone, nullopt,
              sync_pipe.source_bucket_info.bucket, std::nullopt,
              sync_pipe.dest_bucket_info, key, dest_key, versioned_epoch, true,
      ...
      ldout(cct, 0) << "ERROR: " << __func__
                    << ": Too many retries trying to fetch object, possibly a "
                       "bug: bucket="
                    << sync_pipe.source_bucket_info.bucket.get_key()
                    << " key=" << key << dendl;
...
RGWCoroutine *RGWDefaultDataSyncModule::sync_object(
    const DoutPrefixProvider *dpp, RGWDataSyncCtx *sc,
    rgw_bucket_sync_pipe &sync_pipe, rgw_obj_key &key,
    std::optional<uint64_t> versioned_epoch,
    const rgw_zone_set_entry &source_trace_entry, rgw_zone_set *zones_trace) {
  bool stat_follow_olh = false;
  return new RGWObjFetchCR(sc, sync_pipe, key, std::nullopt, versioned_epoch,
                           stat_follow_olh, source_trace_entry, zones_trace);
}

RGWCoroutine *RGWDefaultDataSyncModule::remove_object(
    const DoutPrefixProvider *dpp, RGWDataSyncCtx *sc,
    rgw_bucket_sync_pipe &sync_pipe, rgw_obj_key &key, real_time &mtime,
    bool versioned, uint64_t versioned_epoch, rgw_zone_set *zones_trace) {
  auto sync_env = sc->env;
  return new RGWRemoveObjCR(
      sync_env->dpp, sync_env->async_rados, sync_env->driver, sc->source_zone,
      sync_pipe.dest_bucket_info, key, versioned, versioned_epoch, NULL, NULL,
      false, &mtime, zones_trace);
}

RGWCoroutine *RGWDefaultDataSyncModule::create_delete_marker(
    const DoutPrefixProvider *dpp, RGWDataSyncCtx *sc,
    rgw_bucket_sync_pipe &sync_pipe, rgw_obj_key &key, real_time &mtime,
  ...
  return new RGWRemoveObjCR(
      sync_env->dpp, sync_env->async_rados, sync_env->driver, sc->source_zone,
      sync_pipe.dest_bucket_info, key, versioned, versioned_epoch, &owner.id,
      &owner.display_name, true, &mtime, zones_trace);
}
...
public:
  ...
  RGWDataSyncModule *get_data_handler() override { return &data_handler; }
  RGWMetadataHandler *
  alloc_bucket_meta_handler(librados::Rados &rados) override {
    return RGWArchiveBucketMetaHandlerAllocator::alloc(rados);
  }
  RGWBucketInstanceMetadataHandlerBase *
  alloc_bucket_instance_meta_handler(rgw::sal::Driver *driver) override {
    return RGWArchiveBucketInstanceMetaHandlerAllocator::alloc(driver);
  }
};

int RGWArchiveSyncModule::create_instance(const DoutPrefixProvider *dpp,
                                          CephContext *cct,
                                          const JSONFormattable &config,
                                          RGWSyncModuleInstanceRef *instance) {
...
RGWCoroutine *RGWArchiveDataSyncModule::sync_object(
    const DoutPrefixProvider *dpp, RGWDataSyncCtx *sc,
    rgw_bucket_sync_pipe &sync_pipe, rgw_obj_key &key,
    std::optional<uint64_t> versioned_epoch,
    const rgw_zone_set_entry &source_trace_entry, rgw_zone_set *zones_trace) {
  auto sync_env = sc->env;
  ldout(sc->cct, 5) << "SYNC_ARCHIVE: sync_object: b="
                    << sync_pipe.info.source_bs.bucket << " k=" << key
...
RGWCoroutine *RGWArchiveDataSyncModule::remove_object(
    const DoutPrefixProvider *dpp, RGWDataSyncCtx *sc,
    rgw_bucket_sync_pipe &sync_pipe, rgw_obj_key &key, real_time &mtime,
    bool versioned, uint64_t versioned_epoch, rgw_zone_set *zones_trace) {
  ldout(sc->cct, 0) << "SYNC_ARCHIVE: remove_object: b="
                    << sync_pipe.info.source_bs.bucket << " k=" << key
                    << " versioned_epoch=" << versioned_epoch << dendl;
  return NULL;
}

RGWCoroutine *RGWArchiveDataSyncModule::create_delete_marker(
    const DoutPrefixProvider *dpp, RGWDataSyncCtx *sc,
    rgw_bucket_sync_pipe &sync_pipe, rgw_obj_key &key, real_time &mtime,
    rgw_bucket_entry_owner &owner, bool versioned, uint64_t versioned_epoch,
    rgw_zone_set *zones_trace) {
  ldout(sc->cct, 0) << "SYNC_ARCHIVE: create_delete_marker: b="
                    << sync_pipe.info.source_bs.bucket << " k=" << key
                    << " mtime=" << mtime << " versioned=" << versioned
                    << " versioned_epoch=" << versioned_epoch << dendl;
  auto sync_env = sc->env;
  return new RGWRemoveObjCR(
      sync_env->dpp, sync_env->async_rados, sync_env->driver, sc->source_zone,
...
public:
  RGWDataSyncControlCR(RGWDataSyncCtx *_sc, uint32_t _num_shards,
                       RGWSyncTraceNodeRef &_tn_parent)
      : RGWBackoffControlCR(_sc->cct, exit_on_error), sc(_sc),
        sync_env(_sc->env), num_shards(_num_shards) {
...
void RGWRemoteDataLog::wakeup(int shard_id,
...
int RGWRemoteDataLog::run_sync(const DoutPrefixProvider *dpp, int num_shards) {
  ...
  const auto &control_pool =
      sc.env->driver->svc()->zone->get_zone_params().control_pool;
  char buf[data_sync_bids_oid.size() + sc.source_zone.id.size() + 16];
  snprintf(buf, sizeof(buf), "%s.%s", data_sync_bids_oid.c_str(),
           sc.source_zone.id.c_str());
...
CephContext *RGWDataSyncStatusManager::get_cct() const { return driver->ctx(); }

int RGWDataSyncStatusManager::init(const DoutPrefixProvider *dpp) {
  RGWZone *zone_def;

  if (!(zone_def = driver->svc()->zone->find_zone(source_zone))) {
    ldpp_dout(this, 0) << "ERROR: failed to find zone config info for zone="
                       << source_zone << dendl;
    return -EIO;
  }

  if (!driver->svc()->sync_modules->get_manager()->supports_data_export(
          zone_def->tier_type)) {
    return -ENOTSUP;
  }
  ...
  if (!conn) {
    ldpp_dout(this, 0) << "connection object to zone " << source_zone
                       << " does not exist" << dendl;
    return -EINVAL;
  }

  error_logger = new RGWSyncErrorLogger(driver, RGW_SYNC_ERROR_LOG_SHARD_PREFIX,
                                        ERROR_LOGGER_SHARDS);

  int r = source_log.init(source_zone, conn, error_logger,
                          driver->getRados()->get_sync_tracer(), sync_module,
                          counters);
  if (r < 0) {
  ...
  if (r < 0) {
    ldpp_dout(this, 5) << "ERROR: master.read_log_info() returned r=" << r
                       << dendl;
    finalize();
    return r;
  }
  ...
  for (int i = 0; i < num_shards; i++) {
    shard_objs[i] =
        rgw_raw_obj(zone_params.log_pool, shard_obj_name(source_zone, i));
  }
...
unsigned RGWDataSyncStatusManager::get_subsys() const { return dout_subsys; }
...
string
RGWDataSyncStatusManager::sync_status_oid(const rgw_zone_id &source_zone) {
  char buf[datalog_sync_status_oid_prefix.size() + source_zone.id.size() + 16];
  snprintf(buf, sizeof(buf), "%s.%s", datalog_sync_status_oid_prefix.c_str(),
           source_zone.id.c_str());
...
string RGWDataSyncStatusManager::shard_obj_name(const rgw_zone_id &source_zone,
                                                int shard_id) {
  char
      buf[datalog_sync_status_shard_prefix.size() + source_zone.id.size() + 16];
  snprintf(buf, sizeof(buf), "%s.%s.%d",
           datalog_sync_status_shard_prefix.c_str(), source_zone.id.c_str(),
           shard_id);
...
public:
  RGWInitBucketShardSyncStatusCoroutine(
      RGWDataSyncCtx *_sc, const rgw_bucket_sync_pair_info &_sync_pair,
      rgw_bucket_shard_sync_info &_status, uint64_t gen,
      const BucketIndexShardsManager &_marker_mgr,
      RGWObjVersionTracker &objv_tracker, bool exclusive)
      : RGWCoroutine(_sc->cct), sc(_sc), sync_env(_sc->env),
        sync_pair(_sync_pair),
        sync_status_oid(RGWBucketPipeSyncStatusManager::inc_status_oid(
            sc->source_zone, _sync_pair, gen)),
        status(_status), objv_tracker(objv_tracker), marker_mgr(_marker_mgr),
        exclusive(exclusive) {}

  int operate(const DoutPrefixProvider *dpp) override {
    reenter(this) {
      yield {
        rgw_raw_obj obj(sync_env->svc->zone->get_zone_params().log_pool,
                        sync_status_oid);

        // whether or not to do full sync, incremental sync will follow anyway
        if (sync_env->sync_module->should_full_sync()) {
          const auto max_marker =
              marker_mgr.get(sync_pair.source_bs.shard_id, "");
        ...
        call(new RGWSimpleRadosWriteAttrsCR(dpp, sync_env->driver, obj, attrs,
                                            &objv_tracker, exclusive));
      }

      if (retcode < 0) {
        ldout(cct, 20) << "ERROR: init marker position failed. error: "
                       << retcode << dendl;
        return set_cr_error(retcode);
      }
      ldout(cct, 20) << "init marker position: " << status.inc_marker.position
                     << ". written to shard status object: " << sync_status_oid
                     << dendl;
      return set_cr_done();
    }
    return 0;
...
template <class T>
static bool decode_attr(CephContext *cct, map<string, bufferlist> &attrs,
  ...
  try {
    ...
    ldout(cct, 0) << "ERROR: failed to decode attribute: " << attr_name
                  << dendl;
    return false;
  }
  return true;
...
void rgw_bucket_shard_sync_info::decode_from_attrs(
    CephContext *cct, map<string, bufferlist> &attrs) {
  if (!decode_attr(cct, attrs, BUCKET_SYNC_ATTR_PREFIX "state", &state)) {
    decode_attr(cct, attrs, "state", &state);
  }
  if (!decode_attr(cct, attrs, BUCKET_SYNC_ATTR_PREFIX "inc_marker",
                   &inc_marker)) {
    decode_attr(cct, attrs, "inc_marker", &inc_marker);
  }
}

void rgw_bucket_shard_sync_info::encode_all_attrs(
    map<string, bufferlist> &attrs) {
  encode_state_attr(attrs);
  inc_marker.encode_attr(attrs);
}

void rgw_bucket_shard_sync_info::encode_state_attr(
    map<string, bufferlist> &attrs) {
  using ceph::encode;
  encode(state, attrs[BUCKET_SYNC_ATTR_PREFIX "state"]);
}

void rgw_bucket_shard_full_sync_marker::encode_attr(
    map<string, bufferlist> &attrs) {
  using ceph::encode;
  encode(*this, attrs[BUCKET_SYNC_ATTR_PREFIX "full_marker"]);
}

void rgw_bucket_shard_inc_sync_marker::encode_attr(
    map<string, bufferlist> &attrs) {
  using ceph::encode;
...
public:
  RGWReadBucketPipeSyncStatusCoroutine(
      RGWDataSyncCtx *_sc, const rgw_bucket_sync_pair_info &sync_pair,
      rgw_bucket_shard_sync_info *_status, RGWObjVersionTracker *objv_tracker,
      uint64_t gen)
      : RGWCoroutine(_sc->cct), sc(_sc), sync_env(_sc->env),
        oid(RGWBucketPipeSyncStatusManager::inc_status_oid(sc->source_zone,
                                                           sync_pair, gen)),
...
int RGWReadBucketPipeSyncStatusCoroutine::operate(
    const DoutPrefixProvider *dpp) {
  reenter(this) {
    yield call(new RGWSimpleRadosReadAttrsCR(
        dpp, sync_env->driver,
    ...
    if (retcode < 0) {
      ldpp_dout(dpp, 0) << "ERROR: failed to call fetch bucket shard info oid="
                        << oid << " ret=" << retcode << dendl;
      return set_cr_error(retcode);
    }
...
class CheckBucketShardStatusIsIncremental
    : public RGWReadBucketPipeSyncStatusCoroutine {
  bool *result;
  rgw_bucket_shard_sync_info status;

public:
  CheckBucketShardStatusIsIncremental(
      RGWDataSyncCtx *sc, const rgw_bucket_sync_pair_info &sync_pair,
      bool *result)
      : RGWReadBucketPipeSyncStatusCoroutine(sc, sync_pair, &status, nullptr,
                                             0 /*no gen in compat mode*/),
        result(result) {}
...
public:
  CheckAllBucketShardStatusIsIncremental(
      RGWDataSyncCtx *sc, const rgw_bucket_sync_pair_info &sync_pair,
      int num_shards, bool *result)
      : RGWShardCollectCR(sc->cct, initial_concurrent_shards), sc(sc),
        sync_pair(sync_pair), num_shards(num_shards), result(result) {}
  ...
  bool spawn_next() override {
    ...
    spawn(new CheckBucketShardStatusIsIncremental(sc, sync_pair, result),
          false);
    return true;
  }
...
private:
  int handle_result(int r) override {
    if (r < 0) {
      ldout(cct, 4) << "failed to read bucket shard status: " << cpp_strerror(r)
                    << dendl;
    } else if (shard == 0) {
...
public:
  InitBucketShardStatusCR(RGWDataSyncCtx *sc,
                          const rgw_bucket_sync_pair_info &pair, uint64_t gen,
                          const BucketIndexShardsManager &marker_mgr)
  ...
  int operate(const DoutPrefixProvider *dpp) {
    reenter(this) {
      ...
      objv.generate_new_write_ver(cct);
      yield call(new RGWInitBucketShardSyncStatusCoroutine(
          sc, pair, status, gen, marker_mgr, objv, false));
      if (retcode < 0) {
        return set_cr_error(retcode);
      }
      return set_cr_done();
...
class InitBucketShardStatusCollectCR : public RGWShardCollectCR {
  ...
  int handle_result(int r) override {
    if (r < 0) {
      ldout(cct, 4) << "failed to init bucket shard status: " << cpp_strerror(r)
                    << dendl;
    }
    return r;
  }
...
public:
  InitBucketShardStatusCollectCR(RGWDataSyncCtx *sc,
                                 ...
                                 const BucketIndexShardsManager &marker_mgr,
                                 int num_shards)
      : RGWShardCollectCR(sc->cct, max_concurrent_shards), sc(sc),
        sync_pair(sync_pair), gen(gen), marker_mgr(marker_mgr),
...
public:
  RemoveBucketShardStatusCR(RGWDataSyncCtx *sc,
                            const rgw_bucket_sync_pair_info &sync_pair,
                            uint64_t gen)
      : RGWCoroutine(sc->cct), sc(sc), sync_env(sc->env), sync_pair(sync_pair),
        obj(sync_env->svc->zone->get_zone_params().log_pool,
            RGWBucketPipeSyncStatusManager::inc_status_oid(sc->source_zone,
                                                           sync_pair, gen)) {}
  ...
  int operate(const DoutPrefixProvider *dpp) override {
    reenter(this) {
      ...
      if (retcode < 0 && retcode != -ENOENT) {
        ldout(cct, 20) << "ERROR: failed to remove bucket shard status for: "
                       << sync_pair << ". with error: " << retcode << dendl;
        return set_cr_error(retcode);
      }
      ldout(cct, 20) << "removed bucket shard status object: " << obj.oid
                     << dendl;
      return set_cr_done();
    }
    return 0;
...
public:
  RemoveBucketShardStatusCollectCR(RGWDataSyncCtx *sc,
                                   const rgw_bucket_sync_pair_info &sync_pair,
                                   uint64_t gen, int num_shards)
      : RGWShardCollectCR(sc->cct, max_concurrent_shards), sc(sc),
        sync_env(sc->env), sync_pair(sync_pair), gen(gen),
        num_shards(num_shards) {}
...
public:
  InitBucketFullSyncStatusCR(
      RGWDataSyncCtx *sc, const rgw_bucket_sync_pair_info &sync_pair,
      const rgw_raw_obj &status_obj, rgw_bucket_sync_status &status,
      RGWObjVersionTracker &objv, const RGWBucketInfo &source_info,
      bool check_compat, const rgw_bucket_index_marker_info &info)
      : RGWCoroutine(sc->cct), sc(sc), sync_env(sc->env), sync_pair(sync_pair),
        status_obj(status_obj), status(status), objv(objv),
        source_info(source_info), check_compat(check_compat), info(info) {}
    ...
    reenter(this) {
      ...
      if (info.oldest_gen == 0) {
        if (check_compat) {
          ...
          // try to convert existing per-shard incremental status for backward
          // compatibility
          if (source_info.layout.logs.empty() ||
              source_info.layout.logs.front().gen > 0) {
            ldpp_dout(dpp, 20)
                << "no generation zero when checking compatibility" << dendl;
            no_zero = true;
          } else if (auto &log = source_info.layout.logs.front();
                     log.layout.type != rgw::BucketLogType::InIndex) {
            ldpp_dout(dpp, 20)
                << "unrecognized log layout type when checking compatibility "
                << log.layout.type << dendl;
            no_zero = true;
          }
          if (!no_zero) {
            yield {
              ...
              call(new CheckAllBucketShardStatusIsIncremental(
                  sc, sync_pair, num_shards0, &all_incremental));
            }
            if (retcode < 0) {
              return set_cr_error(retcode);
            }
      ...
      if (status.state != BucketSyncState::Incremental) {
        ...
        yield {
          ...
          call(new InitBucketShardStatusCollectCR(
      ...
      ldout(cct, 20) << "writing bucket sync status during init. state="
                     << status.state << ". marker=" << status.full.position
                     << dendl;

      // write bucket sync status
      using CR = RGWSimpleRadosWriteCR<rgw_bucket_sync_status>;
      yield call(
          new CR(dpp, sync_env->driver, status_obj, status, &objv, false));
      if (retcode < 0) {
...
public:
  RGWReadRecoveringBucketShardsCoroutine(RGWDataSyncCtx *_sc,
                                         const int _shard_id,
                                         set<string> &_recovering_buckets,
                                         const int _max_entries)
      : RGWCoroutine(_sc->cct), sc(_sc), sync_env(_sc->env),
        driver(sync_env->driver), shard_id(_shard_id),
        max_entries(_max_entries), recovering_buckets(_recovering_buckets),
...
int RGWReadRecoveringBucketShardsCoroutine::operate(
  ...
  reenter(this) {
    ...
    do {
      ...
      yield call(new RGWRadosGetOmapKeysCR(
          driver,
          rgw_raw_obj(sync_env->svc->zone->get_zone_params().log_pool,
                      error_oid),
      ...
      for (const std::string &key : error_entries) {
        ...
        if (int r = rgw::error_repo::decode_key(key, bs, gen); r < 0) {
          ...
          recovering_buckets.insert(
              fmt::format("{}[{}]", bucket_shard_str{bs}, *gen));
        } else {
          recovering_buckets.insert(
              fmt::format("{}[full]", bucket_shard_str{bs}));
        }
      }
...
public:
  RGWReadPendingBucketShardsCoroutine(RGWDataSyncCtx *_sc, const int _shard_id,
                                      ...
                                      rgw_data_sync_marker *_sync_marker,
                                      const int _max_entries)
      : RGWCoroutine(_sc->cct), sc(_sc), sync_env(_sc->env),
        driver(sync_env->driver), shard_id(_shard_id),
        max_entries(_max_entries), pending_buckets(_pending_buckets),
...
int RGWReadPendingBucketShardsCoroutine::operate(
  ...
  reenter(this) {
    ...
    do {
      yield call(new RGWReadRemoteDataLogShardCR(
          sc, shard_id, marker, &next_marker, &log_entries, &truncated));
...
int RGWRemoteDataLog::read_shard_status(const DoutPrefixProvider *dpp,
                                        int shard_id,
                                        set<string> &pending_buckets,
                                        set<string> &recovering_buckets,
                                        rgw_data_sync_marker *sync_marker,
  ...
  // cannot run concurrently with run_sync(), so run in a separate manager
  RGWCoroutinesManager crs(driver->ctx(),
                           driver->getRados()->get_cr_registry());
  ...
  RGWCoroutinesStack *recovering_stack =
      new RGWCoroutinesStack(driver->ctx(), &crs);
  recovering_stack->call(new RGWReadRecoveringBucketShardsCoroutine(
      &sc_local, shard_id, recovering_buckets, max_entries));
  stacks.push_back(recovering_stack);
  RGWCoroutinesStack *pending_stack =
      new RGWCoroutinesStack(driver->ctx(), &crs);
  pending_stack->call(new RGWReadPendingBucketShardsCoroutine(
      &sc_local, shard_id, pending_buckets, sync_marker, max_entries));
...
struct bucket_list_entry {
  ...
  bucket_list_entry()
      : delete_marker(false), is_latest(false), size(0), versioned_epoch(0) {}
  ...
  int operate(const DoutPrefixProvider *dpp) override {
    reenter(this) {
      yield {
        ...
        // Log the HTTP parameters being sent
        ldpp_dout(dpp, 0)
            << "RGWListRemoteBucketCR: Sending bucket list request with params:"
            << dendl;
        for (int i = 0; pairs[i].key != NULL; i++) {
          if (pairs[i].val) {
            ldpp_dout(dpp, 0)
                << "  " << pairs[i].key << " = " << pairs[i].val << dendl;
          } else {
            ldpp_dout(dpp, 0) << "  " << pairs[i].key << " = (null)" << dendl;
          }
        }
        ldpp_dout(dpp, 0) << "  marker_position.name = " << marker_position.name
                          << dendl;
        ldpp_dout(dpp, 0) << "  marker_position.instance = "
                          << marker_position.instance << dendl;
 
        string p = string("/") + bs.bucket.get_key(':', 0);
        call(new RGWReadRESTResourceCR<bucket_list_result>(
            sync_env->cct, sc->conn, sync_env->http_manager, p, pairs, result));
      }
      if (retcode < 0) {
        return set_cr_error(retcode);
      }
      
      // Log the response from Primary
      ldpp_dout(dpp, 0) << "RGWListRemoteBucketCR: Received response from Primary:" << dendl;
      ldpp_dout(dpp, 0) << "  Name: " << result->name << dendl;
      ldpp_dout(dpp, 0) << "  Prefix: " << result->prefix << dendl;
      ldpp_dout(dpp, 0) << "  KeyMarker: " << result->key_marker << dendl;
      ldpp_dout(dpp, 0) << "  VersionIdMarker: " << result->version_id_marker << dendl;
      ldpp_dout(dpp, 0) << "  IsTruncated: " << (result->is_truncated ? "true" : "false") << dendl;
      ldpp_dout(dpp, 0) << "  MaxKeys: " << result->max_keys << dendl;
      ldpp_dout(dpp, 0) << "  Entries count: " << result->entries.size() << dendl;
      
      // Log first and last entry if exists
      if (!result->entries.empty()) {
        ldpp_dout(dpp, 0) << "  First entry key: " << result->entries.front().key.name << dendl;
        ldpp_dout(dpp, 0) << "  Last entry key: " << result->entries.back().key.name << dendl;
        
        // Check if the last entry contains encoded characters
        const string& last_key = result->entries.back().key.name;
        if (last_key.find("%2F") != string::npos || last_key.find("%3D") != string::npos) {
          ldpp_dout(dpp, 0) << "  NOTE: Last entry key contains URL-encoded characters" << dendl;
        }
      }
...
public:
  RGWListBucketIndexLogCR(RGWDataSyncCtx *_sc, const rgw_bucket_shard &bs,
                          string &_marker, uint64_t _generation,
                          bilog_list_result *_result)
      : RGWCoroutine(_sc->cct), sc(_sc), sync_env(_sc->env),
  ...
  int operate(const DoutPrefixProvider *dpp) override {
    reenter(this) {
      ...
      yield {
        ...
        call(new RGWReadRESTResourceCR<bilog_list_result>(
            sync_env->cct, sc->conn, sync_env->http_manager, "/admin/log",
            pairs, result));
      }
      timer.reset();
      if (retcode < 0) {
...
class RGWBucketFullSyncMarkerTrack
    : public RGWSyncShardMarkerTrack<rgw_obj_key, rgw_obj_key> {
  RGWDataSyncCtx *sc;
  RGWDataSyncEnv *sync_env;
...
public:
  RGWBucketFullSyncMarkerTrack(RGWDataSyncCtx *_sc,
                               ...
                               RGWSyncTraceNodeRef tn,
                               RGWObjVersionTracker &objv_tracker)
      : RGWSyncShardMarkerTrack(BUCKET_SYNC_UPDATE_MARKER_WINDOW), sc(_sc),
        sync_env(_sc->env), status_obj(status_obj), sync_status(sync_status),
  ...
  RGWCoroutine *store_marker(const rgw_obj_key &new_marker, uint64_t index_pos,
    ...
    tn->log(20, SSTR("updating marker oid=" << status_obj.oid
                                            << " marker=" << new_marker));
    return new RGWSimpleRadosWriteCR<rgw_bucket_sync_status>(
        sync_env->dpp, sync_env->driver, status_obj, sync_status,
        &objv_tracker);
  }
...
public:
  RGWWriteBucketShardIncSyncStatus(
      RGWDataSyncEnv *sync_env, const rgw_raw_obj &obj,
      const rgw_bucket_shard_inc_sync_marker &sync_marker,
      ceph::real_time *stable_timestamp, RGWObjVersionTracker &objv_tracker)
...
class RGWBucketIncSyncShardMarkerTrack
    : public RGWSyncShardMarkerTrack<string, rgw_obj_key> {
  RGWDataSyncCtx *sc;
  RGWDataSyncEnv *sync_env;
...
public:
  RGWBucketIncSyncShardMarkerTrack(
      RGWDataSyncCtx *_sc, const string &_marker_oid,
      const rgw_bucket_shard_inc_sync_marker &_marker, RGWSyncTraceNodeRef tn,
      RGWObjVersionTracker &objv_tracker, ceph::real_time *stable_timestamp)
      : RGWSyncShardMarkerTrack(BUCKET_SYNC_UPDATE_MARKER_WINDOW), sc(_sc),
        sync_env(_sc->env),
  ...
  RGWCoroutine *store_marker(const string &new_marker, uint64_t index_pos,
    ...
    tn->log(20, SSTR("updating marker marker_oid="
                     << obj.oid << " marker=" << new_marker
                     << " timestamp=" << timestamp));
   ...
   * Also, we should make sure that we don't run concurrent operations on the
   * same key with different ops.
   */
...
public:
  RGWBucketSyncSingleEntryCR(
    ...
    set_description() << "bucket sync single entry (source_zone="
                      << sc->source_zone << ") b=" << ss.str()
                      << " log_entry=" << entry_marker << " op=" << (int)op
                      << " op_state=" << (int)op_state;
    set_status("init");

    tn = sync_env->sync_tracer->add_node(_tn_parent, "entry", SSTR(key));

    tn->log(20, SSTR("bucket sync single entry (source_zone="
                     << sc->source_zone << ") b=" << ss.str()
                     << " log_entry=" << entry_marker << " op=" << (int)op
                     << " op_state=" << (int)op_state));
    error_injection =
        (sync_env->cct->_conf->rgw_sync_data_inject_err_probability > 0);

    data_sync_module = sync_env->sync_module->get_data_handler();

    source_trace_entry.zone = sc->source_zone.id;
    source_trace_entry.location_key =
        _sync_pipe.info.source_bs.bucket.get_key();

    zones_trace = _zones_trace;
    zones_trace.insert(sync_env->svc->zone->get_zone().id,
                       _sync_pipe.info.dest_bucket.get_key());
    ...
    reenter(this) {
      ...
      do {
        yield {
          ...
          if (error_injection &&
              rand() % 10000 <
                  cct->_conf->rgw_sync_data_inject_err_probability * 10000.0) {
            ...
            tn->log(5, SSTR("bucket sync: sync obj: "
            ...
            call(data_sync_module->sync_object(
                dpp, sc, sync_pipe, key, versioned_epoch, source_trace_entry,
                &zones_trace));
          } else if (op == CLS_RGW_OP_DEL || op == CLS_RGW_OP_UNLINK_INSTANCE) {
            set_status("removing obj");
            if (versioned_epoch) {
              pretty_print(sc->env,
                           "Deleting object s3://{}/{} version {} in sync from "
                           "zone {}\n",
                           bs.bucket.name, key, *versioned_epoch, zone_name);
            } else {
              pretty_print(sc->env,
            ...
            tn->log(10, SSTR("removing obj: " << sc->source_zone << "/"
                                              << bs.bucket << "/" << key << "["
                                              << versioned_epoch.value_or(0)
                                              << "]"));
            call(data_sync_module->remove_object(
                dpp, sc, sync_pipe, key, timestamp, versioned,
                versioned_epoch.value_or(0), &zones_trace));
            // our copy of the object is more recent, continue as if it
            // succeeded
          } else if (op == CLS_RGW_OP_LINK_OLH_DM) {
            set_status("creating delete marker");
            tn->log(10,
                    SSTR("creating delete marker: obj: "
                         << sc->source_zone << "/" << bs.bucket << "/" << key
                         << "[" << versioned_epoch.value_or(0) << "]"));
            call(data_sync_module->create_delete_marker(
                dpp, sc, sync_pipe, key, timestamp, owner, versioned,
                versioned_epoch.value_or(0), &zones_trace));
          }
          tn->set_resource_name(
              SSTR(bucket_str_noinstance(bs.bucket) << "/" << key));
        }
        if (retcode == -ERR_PRECONDITION_FAILED) {
          pretty_print(sc->env,
                       "Skipping object s3://{}/{} in sync from zone {}\n",
                       bs.bucket.name, key, zone_name);
          set_status("Skipping object sync: precondition failed (object "
                     "contains newer change or policy doesn't allow sync)");
          tn->log(0, "Skipping object sync: precondition failed (object "
                     "contains newer change or policy doesn't allow sync)");
          retcode = 0;
        }
      ...
      if (retcode < 0 && retcode != -ENOENT) {
        ...
        tn->log(0, SSTR("ERROR: failed to sync object: " << bucket_shard_str{bs}
                                                         << "/" << key.name));
      ...
      if (!error_ss.str().empty()) {
        yield call(sync_env->error_logger->log_error_cr(
            dpp, sc->conn->get_remote_id(), "data", error_ss.str(), -retcode,
...
class RGWBucketFullSyncCR : public RGWCoroutine {
  ...
  struct _prefix_handler {
    ...
    bool revalidate_marker(rgw_obj_key *marker) {
      if (cur_prefix && boost::starts_with(marker->name, *cur_prefix)) {
        return true;
      }
      if (!rules) {
        return false;
    ...
    bool check_key_handled(const rgw_obj_key &key) {
      ...
      if (cur_prefix && boost::starts_with(key.name, *cur_prefix)) {
        return true;
      }
...
public:
  RGWBucketFullSyncCR(RGWDataSyncCtx *_sc, rgw_bucket_sync_pipe &_sync_pipe,
                      const rgw_raw_obj &status_obj,
                      boost::intrusive_ptr<const RGWContinuousLeaseCR> lease_cr,
                      rgw_bucket_sync_status &sync_status,
      ...
      : RGWCoroutine(_sc->cct), sc(_sc), sync_env(_sc->env),
        ...
        bs(_sync_pipe.info.source_bs), lease_cr(std::move(lease_cr)),
    ...
    zones_trace.insert(sc->source_zone.id,
                       sync_pipe.info.dest_bucket.get_key());
  ...
  reenter(this) {
    ...
    do {
      if (lease_cr && !lease_cr->is_locked()) {
        ...
        yield call(marker_tracker.flush());
        if (retcode < 0) {
          tn->log(0, SSTR("ERROR: bucket full sync marker_tracker.flush() "
                          "returned retcode="
                          << retcode));
          return set_cr_error(retcode);
        }
      ...
      if (!prefix_handler.revalidate_marker(&list_marker)) {
        set_status()
            << "finished iterating over all available prefixes: last marker="
            << list_marker;
        tn->log(
            20,
            SSTR("finished iterating over all available prefixes: last marker="
                 << list_marker));
        break;
      }
      ...
      for (; entries_iter != list_result.entries.end(); ++entries_iter) {
        ...
        if (!prefix_handler.check_key_handled(entries_iter->key)) {
          set_status() << "skipping entry due to policy rules: "
                       << entries_iter->key;
          tn->log(20, SSTR("skipping entry due to policy rules: "
                           << entries_iter->key));
          continue;
        }
        total_entries++;
        if (!marker_tracker.start(entry->key, total_entries, real_time())) {
          tn->log(0, SSTR("ERROR: cannot start syncing "
                          << entry->key << ". Duplicate entry?"));
        } else {
          using SyncCR = RGWBucketSyncSingleEntryCR<rgw_obj_key, rgw_obj_key>;
          yield spawn(
              new SyncCR(sc, sync_pipe, entry->key,
                         false, /* versioned, only matters for object removal */
                         false, entry->versioned_epoch, entry->mtime,
                         entry->owner, entry->get_modify_op(),
                         CLS_RGW_STATE_COMPLETE, entry->key, &marker_tracker,
                         zones_trace, tn),
              false);
        }
        drain_with_cb(
    ...
    if (retcode < 0) {
      tn->log(
          0,
          SSTR(
              "ERROR: bucket full sync marker_tracker.flush() returned retcode="
              << retcode));
      return set_cr_error(retcode);
    }
...
public:
  RGWBucketShardIsDoneCR(RGWDataSyncCtx *_sc,
                         const rgw_raw_obj &_bucket_status_obj, int _shard_id,
                         const next_bilog_result &_next_log,
                         const uint64_t _gen)
      : RGWCoroutine(_sc->cct), sc(_sc), sync_env(_sc->env),
        bucket_status_obj(_bucket_status_obj), shard_id(_shard_id),
        next_log(_next_log), generation(_gen) {}
  ...
  int operate(const DoutPrefixProvider *dpp) override {
    reenter(this) {
      do {
        ...
        yield call(new ReadCR(dpp, sync_env->driver, bucket_status_obj,
                              &bucket_status, false, &objv_tracker));
        if (retcode < 0) {
          ldpp_dout(dpp, 20)
              << "failed to read bucket shard status: " << cpp_strerror(retcode)
              << dendl;
          return set_cr_error(retcode);
        }
        ...
        if (bucket_status.state != BucketSyncState::Incremental) {
          ...
          ldpp_dout(dpp, 20) << "RGWBucketShardIsDoneCR found sync state = "
                             << bucket_status.state << dendl;
          return set_cr_done();
        }
        ...
        if (bucket_status.incremental_gen != generation) {
          ...
          // retried in error repo if we lost a race
          ldpp_dout(dpp, 20)
              << "RGWBucketShardIsDoneCR expected gen: " << generation
        ...
        yield {
          ...
          ldpp_dout(dpp, 20) << "bucket status incremental gen is "
                             << bucket_status.incremental_gen << dendl;
          using WriteCR = RGWSimpleRadosWriteCR<rgw_bucket_sync_status>;
          call(new WriteCR(dpp, sync_env->driver, bucket_status_obj,
                           bucket_status, &objv_tracker, false));
        }
        if (retcode < 0 && retcode != -ECANCELED) {
          ldpp_dout(dpp, 20)
              << "failed to write bucket sync status: " << cpp_strerror(retcode)
              << dendl;
          return set_cr_error(retcode);
        } else if (retcode >= 0) {
          return set_cr_done();
...
public:
  RGWBucketShardIncrementalSyncCR(
      ...
      boost::intrusive_ptr<const RGWContinuousLeaseCR> lease_cr,
      rgw_bucket_shard_sync_info &sync_info, uint64_t generation,
      RGWSyncTraceNodeRef &_tn_parent, RGWObjVersionTracker &objv_tracker,
      ceph::real_time *stable_timestamp)
      : RGWCoroutine(_sc->cct), sc(_sc), sync_env(_sc->env),
        sync_pipe(_sync_pipe), bs(_sync_pipe.info.source_bs),
        bucket_status_obj(_bucket_status_obj), lease_cr(std::move(lease_cr)),
        sync_info(sync_info), generation(generation),
  ...
  reenter(this) {
    do {
      if (lease_cr && !lease_cr->is_locked()) {
        ...
        yield call(marker_tracker.flush());
        if (retcode < 0) {
          tn->log(0, SSTR("ERROR: incremental sync marker_tracker.flush() "
                          "returned retcode="
                          << retcode));
          return set_cr_error(retcode);
        }
      ...
      tn->log(20, SSTR("listing bilog for incremental sync; position="
                       << sync_info.inc_marker.position));
      set_status() << "listing bilog; position="
                   << sync_info.inc_marker.position;
      yield call(new RGWListBucketIndexLogCR(
          sc, bs, sync_info.inc_marker.position, generation, &extended_result));
      if (retcode < 0 && retcode != -ENOENT) {
        /* wait for all operations to complete */
        drain_all();
        yield spawn(marker_tracker.flush(), true);
      ...
      for (; entries_iter != entries_end; ++entries_iter) {
        ...
        if (e.op == RGWModifyOp::CLS_RGW_OP_SYNCSTOP) {
          ldpp_dout(dpp, 20) << "syncstop at: " << e.timestamp
                             << ". marker: " << e.id << dendl;
          syncstopped = true;
        ...
        if (e.op == RGWModifyOp::CLS_RGW_OP_RESYNC) {
          ldpp_dout(dpp, 20) << "syncstart at: " << e.timestamp
                             << ". marker: " << e.id << dendl;
          continue;
        }
      ...
      for (; entries_iter != entries_end; ++entries_iter) {
        ...
        {
          ssize_t p =
              entry->id.find('#'); /* entries might have explicit shard info in
                                      them, e.g., 6#00000000004.94.3 */
          if (p < 0) {
            cur_id = entry->id;
          } else {
        ...
        if (entry->op == RGWModifyOp::CLS_RGW_OP_SYNCSTOP ||
            entry->op == RGWModifyOp::CLS_RGW_OP_RESYNC) {
          ldpp_dout(dpp, 20)
              << "detected syncstop or resync on " << entries_iter->timestamp
        ...
        if (!key.set(rgw_obj_index_key{entry->object, entry->instance})) {
          set_status() << "parse_raw_oid() on " << entry->object
                       << " returned false, skipping entry";
          tn->log(20,
                  SSTR("parse_raw_oid() on "
                       << entry->object << " returned false, skipping entry"));
          marker_tracker.try_update_high_marker(cur_id, 0, entry->timestamp);
          continue;
        }

        tn->log(20, SSTR("parsed entry: id="
                         << cur_id << " iter->object=" << entry->object
                         << " iter->instance=" << entry->instance
        ...
        set_status() << "got entry.id=" << cur_id << " key=" << key
                     << " op=" << (int)entry->op;
        if (entry->op == CLS_RGW_OP_CANCEL) {
          set_status() << "canceled operation, skipping";
          tn->log(20,
                  SSTR("skipping object: " << bucket_shard_str{bs} << "/" << key
                                           << ": canceled operation"));
          marker_tracker.try_update_high_marker(cur_id, 0, entry->timestamp);
          continue;
        }
        ...
        if (make_pair<>(entry->timestamp, entry->op) !=
            squash_map[make_pair(entry->object, entry->instance)]) {
        ...
        tn->set_flag(RGW_SNS_FLAG_ACTIVE);
        tn->log(20,
                SSTR("syncing object: " << bucket_shard_str{bs} << "/" << key));
        ...
        while (!marker_tracker.can_do_op(key, has_olh_epoch(entry->op))) {
          ...
          while (again) {
            ...
            if (ret < 0) {
              tn->log(0, SSTR("ERROR: a child operation returned error (ret="
                              << ret << ")"));
              sync_status = ret;
        ...
        if (!marker_tracker.index_key_to_marker(key, cur_id,
                                                has_olh_epoch(entry->op))) {
          set_status() << "can't do op, sync already in progress for object";
          tn->log(20, SSTR("skipping sync of entry: "
                           << cur_id << ":" << key
    ...
    if (retcode < 0) {
      tn->log(
          0,
          SSTR(
              "ERROR: incremental sync marker_tracker.flush() returned retcode="
              << retcode));
      return set_cr_error(retcode);
    }
    if (sync_status < 0) {
    ...
    if (!truncated && extended_result.next_log) {
      yield call(new RGWBucketShardIsDoneCR(sc, bucket_status_obj, bs.shard_id,
                                            *extended_result.next_log,
                                            generation));
      if (retcode < 0) {
      ...
      yield {
        ...
        retcode = rgw_get_rados_ref(
            dpp, sync_env->driver->getRados()->get_rados_handle(),
            marker_tracker.get_obj(), &status_obj);
        if (retcode < 0) {
          return set_cr_error(retcode);
        }
        call(new RGWRadosRemoveOidCR(sync_env->driver, std::move(status_obj)));
        if (retcode < 0) {
          ldpp_dout(dpp, 20) << "failed to remove shard status object: "
                             << cpp_strerror(retcode) << dendl;
          return set_cr_error(retcode);
        }
      }
...
class RGWGetBucketPeersCR : public RGWCoroutine {
  ...
  using pipe_const_iter =
      map<rgw_zone_id, RGWBucketSyncFlowManager::pipe_set>::const_iterator;

  static pair<pipe_const_iter, pipe_const_iter>
  get_pipe_iters(const map<rgw_zone_id, RGWBucketSyncFlowManager::pipe_set> &m,
  ...
  void filter_sources(
    ...
    for (auto i = iters.first; i != iters.second; ++i) {
      for (auto &handler : i->second) {
        if (!handler.specific()) {
          ldpp_dout(sync_env->dpp, 20)
              << __func__ << ": pipe_handler=" << handler << ": skipping"
              << dendl;
          continue;
        }
        if (source_bucket && !source_bucket->match(*handler.source.bucket)) {
          continue;
        }
        ldpp_dout(sync_env->dpp, 20)
            << __func__ << ": pipe_handler=" << handler << ": adding" << dendl;
  ...
  void filter_targets(
      std::optional<rgw_zone_id> target_zone,
      std::optional<rgw_bucket> target_bucket,
      const map<rgw_zone_id, RGWBucketSyncFlowManager::pipe_set> &all_targets,
      rgw_sync_pipe_info_set *result) {
    ldpp_dout(sync_env->dpp, 20)
        << __func__
        << ": target_zone=" << source_zone.value_or(rgw_zone_id("*")).id
    ...
    for (auto i = iters.first; i != iters.second; ++i) {
      for (auto &handler : i->second) {
        if (target_bucket && handler.dest.bucket &&
  ...
  struct GetHintTargets : public RGWGenericAsyncCR::Action {
    ...
    GetHintTargets(RGWDataSyncEnv *_sync_env, const rgw_bucket &_source_bucket)
        : sync_env(_sync_env), source_bucket(_source_bucket) {}
    int operate() override {
      int r = sync_env->svc->bucket_sync->get_bucket_sync_hints(
          sync_env->dpp, source_bucket, nullptr, &targets, null_yield);
      if (r < 0) {
        ldpp_dout(sync_env->dpp, 0)
            << "ERROR: " << __func__
            << "(): failed to fetch bucket sync hints for bucket="
            << source_bucket << dendl;
        return r;
      }
...
public:
  RGWGetBucketPeersCR(RGWDataSyncEnv *_sync_env,
      ...
      : RGWCoroutine(_sync_env->cct), sync_env(_sync_env),
        target_bucket(_target_bucket), source_zone(_source_zone),
        source_bucket(_source_bucket), pipes(_pipes),
        tn(sync_env->sync_tracer->add_node(
            _tn_parent, "get_bucket_peers",
            SSTR("target=" << target_bucket.value_or(rgw_bucket())
                           << ":source=" << target_bucket.value_or(rgw_bucket())
                           << ":source_zone="
                           << source_zone.value_or(rgw_zone_id("*")).id))) {}

  int operate(const DoutPrefixProvider *dpp) override;
};

std::ostream &operator<<(std::ostream &out,
                         std::optional<rgw_bucket_shard> &bs) {
  if (!bs) {
    out << "*";
  } else {
    out << *bs;
...
static RGWCoroutine *sync_bucket_shard_cr(
    RGWDataSyncCtx *sc, boost::intrusive_ptr<const RGWContinuousLeaseCR> lease,
    const rgw_bucket_sync_pair_info &sync_pair, std::optional<uint64_t> gen,
    const RGWSyncTraceNodeRef &tn, ceph::real_time *progress);

RGWRunBucketSourcesSyncCR::RGWRunBucketSourcesSyncCR(
    RGWDataSyncCtx *_sc,
    boost::intrusive_ptr<const RGWContinuousLeaseCR> lease_cr,
    const rgw_bucket_shard &source_bs, const RGWSyncTraceNodeRef &_tn_parent,
    std::optional<uint64_t> gen, ceph::real_time *progress)
...
int RGWRunBucketSourcesSyncCR::operate(const DoutPrefixProvider *dpp) {
  reenter(this) {
    ...
    if (retcode < 0 && retcode != -ENOENT) {
      tn->log(0, SSTR("ERROR: failed to read sync status for bucket. error: "
                      << retcode));
      return set_cr_error(retcode);
    }

    ldpp_dout(dpp, 20) << __func__
                       << "(): requested source_bs=" << sync_pair.source_bs
                       << dendl;

    if (pipes.empty()) {
      ldpp_dout(dpp, 20) << __func__ << "(): no relevant sync pipes found"
                         << dendl;
      return set_cr_done();
    }
    ...
    for (siter = pipes.begin(); siter != pipes.end();
         ++siter, ++cur_shard_progress) {
      ...
      yield_spawn_window(
          sync_bucket_shard_cr(sc, lease_cr, sync_pair, gen, tn,
                               &*cur_shard_progress),
          sc->lcc.adj_concurrency(cct->_conf->rgw_bucket_sync_spawn_window),
          [&](uint64_t stack_id, int ret) {
            if (ret < 0) {
              tn->log(10,
                      SSTR("ERROR: a sync operation returned error: " << ret));
            }
            return ret;
          });
    }
    ...
    if (progress) {
      *progress =
          *std::min_element(shard_progress.begin(), shard_progress.end());
    }
    return set_cr_done();
  }
...
int RGWSyncGetBucketInfoCR::operate(const DoutPrefixProvider *dpp) {
  reenter(this) {
    yield call(new RGWGetBucketInstanceInfoCR(sync_env->async_rados,
                                              sync_env->driver, bucket,
                                              pbucket_info, pattrs, dpp));
    if (retcode == -ENOENT) {
      ...
      yield {
        ...
        meta_sync_env.init(dpp, cct, sync_env->driver,
                           sync_env->svc->zone->get_master_conn(),
                           sync_env->async_rados, sync_env->http_manager,
                           sync_env->error_logger, sync_env->sync_tracer);

        call(new RGWMetaSyncSingleEntryCR(
            &meta_sync_env, raw_key, string() /* no marker */,
            MDLOG_STATUS_COMPLETE, NULL /* no marker tracker */, tn));
      }
      if (retcode < 0) {
        tn->log(0, SSTR("ERROR: failed to fetch bucket instance info for "
                        << bucket_str{bucket}));
        return set_cr_error(retcode);
      }
    ...
    if (retcode < 0) {
      tn->log(0, SSTR("ERROR: failed to retrieve bucket info for bucket="
                      << bucket_str{bucket}));
      return set_cr_error(retcode);
    }
...
void RGWGetBucketPeersCR::update_from_target_bucket_policy() {
  if (!target_policy || !target_policy->policy_handler || !pipes) {
    return;
  }

  auto handler = target_policy->policy_handler.get();

  filter_sources(source_zone, source_bucket, handler->get_sources(), pipes);
...
void RGWGetBucketPeersCR::update_from_source_bucket_policy() {
  if (!source_policy || !source_policy->policy_handler || !pipes) {
    return;
  }

  auto handler = source_policy->policy_handler.get();

  filter_targets(sync_env->svc->zone->get_zone().id, target_bucket,
                 handler->get_targets(), pipes);
...
public:
  RGWSyncGetBucketSyncPolicyHandlerCR(
      RGWDataSyncEnv *_sync_env, std::optional<rgw_zone_id> zone,
      ...
      : RGWCoroutine(_sync_env->cct), sync_env(_sync_env), bucket(_bucket),
        policy(_policy),
        tn(sync_env->sync_tracer->add_node(
            _tn_parent, "get_sync_policy_handler", SSTR(bucket))) {
  ...
  int operate(const DoutPrefixProvider *dpp) override {
    reenter(this) {
      for (i = 0; i < 2; ++i) {
        yield call(new RGWBucketGetSyncPolicyHandlerCR(
            sync_env->async_rados, sync_env->driver, get_policy_params, policy,
            dpp));
        if (retcode < 0 && retcode != -ENOENT) {
          return set_cr_error(retcode);
        }
         ...
         */
        yield call(
            new RGWSyncGetBucketInfoCR(sync_env, bucket, nullptr, nullptr, tn));
        if (retcode < 0) {
          return set_cr_error(retcode);
        }
      }
...
int RGWGetBucketPeersCR::operate(const DoutPrefixProvider *dpp) {
  reenter(this) {
    ...
    if (target_bucket) {
      ...
      yield call(new RGWSyncGetBucketSyncPolicyHandlerCR(
          sync_env, nullopt, *target_bucket, target_policy, tn));
      if (retcode < 0 && retcode != -ENOENT) {
        return set_cr_error(retcode);
      }
    ...
    if (source_bucket && source_zone) {
      ...
      yield call(new RGWSyncGetBucketSyncPolicyHandlerCR(
          sync_env, source_zone, *source_bucket, source_policy, tn));
      if (retcode < 0 && retcode != -ENOENT) {
        return set_cr_error(retcode);
      }

      if (source_policy->policy_handler) {
        auto &opt_bucket_info =
            source_policy->policy_handler->get_bucket_info();
      ...
      if (!target_bucket) {
        get_hint_targets_action =
            make_shared<GetHintTargets>(sync_env, *source_bucket);
        ...
        for (hiter = get_hint_targets_action->targets.begin();
             hiter != get_hint_targets_action->targets.end(); ++hiter) {
          ldpp_dout(dpp, 20) << "Got sync hint for bucket=" << *source_bucket
                             << ": " << hiter->get_key() << dendl;

          target_policy = make_shared<rgw_bucket_get_sync_policy_result>();
          yield call(new RGWSyncGetBucketSyncPolicyHandlerCR(
              sync_env, nullopt, *hiter, target_policy, tn));
          if (retcode < 0 && retcode != -ENOENT) {
            return set_cr_error(retcode);
          }
          update_from_target_bucket_policy();
    ...
    for (siiter = buckets_info.begin(); siiter != buckets_info.end();
         ++siiter) {
...
public:
  RGWSyncBucketShardCR(
      RGWDataSyncCtx *_sc,
      boost::intrusive_ptr<const RGWContinuousLeaseCR> lease_cr,
      const rgw_bucket_sync_pair_info &_sync_pair,
      rgw_bucket_sync_pipe &sync_pipe, bool &bucket_stopped,
      uint64_t generation, const RGWSyncTraceNodeRef &tn,
      ceph::real_time *progress)
      : RGWCoroutine(_sc->cct), sc(_sc), sync_env(_sc->env),
        lease_cr(std::move(lease_cr)), sync_pair(_sync_pair),
        sync_pipe(sync_pipe), bucket_stopped(bucket_stopped),
        generation(generation), progress(progress),
        shard_status_oid(RGWBucketPipeSyncStatusManager::inc_status_oid(
            sc->source_zone, sync_pair, generation)),
        bucket_status_obj(sc->env->svc->zone->get_zone_params().log_pool,
                          RGWBucketPipeSyncStatusManager::full_status_oid(
                              sc->source_zone, sync_pair.source_bs.bucket,
...
int RGWSyncBucketShardCR::operate(const DoutPrefixProvider *dpp) {
  reenter(this) {
    objv_tracker.clear();
    yield call(new RGWReadBucketPipeSyncStatusCoroutine(
        sc, sync_pair, &sync_status, &objv_tracker, generation));
    if (retcode < 0 && retcode != -ENOENT) {
      tn->log(0, SSTR("ERROR: failed to read sync status for bucket. error: "
                      << retcode));
      return set_cr_error(retcode);
    ...
    tn->log(20,
            SSTR("sync status for source bucket shard: " << sync_status.state));
    sync_status.state = rgw_bucket_shard_sync_info::StateIncrementalSync;
    if (progress) {
      *progress = sync_status.inc_marker.timestamp;
    }

    yield call(new RGWBucketShardIncrementalSyncCR(
        sc, sync_pipe, shard_status_oid, bucket_status_obj, lease_cr,
        sync_status, generation, tn, objv_tracker, progress));
    if (retcode < 0) {
      tn->log(5,
              SSTR("incremental sync on bucket failed, retcode=" << retcode));
      return set_cr_error(retcode);
    }
...
public:
  RGWSyncBucketCR(RGWDataSyncCtx *_sc,
      ...
      : RGWCoroutine(_sc->cct), sc(_sc), env(_sc->env),
        data_lease_cr(std::move(lease_cr)), sync_pair(_sync_pair), gen(gen),
        progress(progress), lock_duration(cct->_conf->rgw_sync_lease_period),
        status_obj(env->svc->zone->get_zone_params().log_pool,
                   RGWBucketPipeSyncStatusManager::full_status_oid(
                       sc->source_zone, sync_pair.source_bs.bucket,
                       sync_pair.dest_bucket)),
        tn(env->sync_tracer->add_node(
            _tn_parent, "bucket",
            SSTR(bucket_str{_sync_pair.dest_bucket}
...
static RGWCoroutine *sync_bucket_shard_cr(
    ...
    const rgw_bucket_sync_pair_info &sync_pair, std::optional<uint64_t> gen,
    const RGWSyncTraceNodeRef &tn, ceph::real_time *progress) {
  return new RGWSyncBucketCR(sc, std::move(lease), sync_pair, gen, tn,
                             progress);
}
...
int RGWSyncBucketCR::operate(const DoutPrefixProvider *dpp) {
  reenter(this) {
    ...
    yield call(new RGWSyncGetBucketInfoCR(env, sync_pair.source_bs.bucket,
                                          &sync_pipe.source_bucket_info,
                                          &sync_pipe.source_bucket_attrs, tn));
    if (retcode < 0) {
      tn->log(0, SSTR("ERROR: failed to retrieve bucket info for bucket="
                      << bucket_str{sync_pair.source_bs.bucket}));
      return set_cr_error(retcode);
    }

    yield call(new RGWSyncGetBucketInfoCR(env, sync_pair.dest_bucket,
                                          &sync_pipe.dest_bucket_info,
    ...
    objv.clear();
    yield call(
        new ReadCR(dpp, env->driver, status_obj, &bucket_status, false, &objv));
    if (retcode == -ENOENT) {
      ...
      objv.generate_new_write_ver(cct);
      yield call(new WriteCR(dpp, env->driver, status_obj, bucket_status, &objv,
                             true));
      ...
      if (retcode == -EEXIST) {
        ...
        objv.clear();
        yield call(new ReadCR(dpp, env->driver, status_obj, &bucket_status,
                              false, &objv));
      }
    }
    if (retcode < 0) {
      tn->log(20, SSTR("ERROR: failed to read bucket status object. error: "
                       << retcode));
      return set_cr_error(retcode);
    }

    do {
      tn->log(20, SSTR("sync status for source bucket: "
                       << bucket_status.state << ". lease is: "
                       << (bucket_lease_cr ? "taken" : "not taken")
                       << ". stop indications is: " << bucket_stopped));

      if (bucket_status.state != BucketSyncState::Incremental ||
          bucket_stopped) {

        if (!bucket_lease_cr) {
          bucket_lease_cr.reset(new RGWContinuousLeaseCR(
              env->async_rados, env->driver, status_obj, lock_name,
              lock_duration, this, &sc->lcc));
        ...
        yield call(new RGWReadRemoteBucketIndexLogInfoCR(
            sc, sync_pair.source_bs.bucket, &info));
        if (retcode < 0) {
        ...
        if (info.syncstopped) {
          ...
          if (bucket_status.state == BucketSyncState::Incremental) {
            yield {
              ...
              call(new RemoveBucketShardStatusCollectCR(sc, sync_pair, gen,
                                                        num_shards));
            }
          }
          ...
          yield call(new ReadCR(dpp, env->driver, status_obj, &bucket_status,
                                false, &objv));
          if (retcode < 0) {
            tn->log(20, SSTR("ERROR: failed to read status before writing "
                             "'stopped'. error: "
                             << retcode));
            RELEASE_LOCK(bucket_lease_cr);
            return set_cr_error(retcode);
          ...
          if (bucket_status.state != BucketSyncState::Stopped) {
            ...
            if (retcode < 0) {
              tn->log(20,
                      SSTR("ERROR: failed to write 'stopped' status. error: "
                           << retcode));
              RELEASE_LOCK(bucket_lease_cr);
              return set_cr_error(retcode);
            }
        ...
        if (bucket_stopped) {
          tn->log(20, SSTR("ERROR: switched from 'stop' to 'start' sync. while "
                           "state is: "
                           << bucket_status.state));
      ...
      if (bucket_status.state != BucketSyncState::Incremental) {
        ...
        if (retcode < 0) {
          RELEASE_LOCK(bucket_lease_cr);
          tn->log(20, SSTR("ERROR: reading the status after acquiring the lock "
                           "failed. error: "
                           << retcode));
          return set_cr_error(retcode);
        }
        tn->log(20, SSTR("status after acquiring the lock is: "
                         << bucket_status.state));

        yield call(new InitBucketFullSyncStatusCR(
            sc, sync_pair, status_obj, bucket_status, objv,
            sync_pipe.source_bucket_info, init_check_compat, info));
      ...
      if (bucket_status.state == BucketSyncState::Full) {
        ...
        yield call(new RGWBucketFullSyncCR(sc, sync_pipe, status_obj,
                                           bucket_lease_cr, bucket_status, tn,
                                           objv));
        if (retcode < 0) {
      ...
      if (bucket_status.state == BucketSyncState::Incremental) {
        ...
        if (gen) {
          ...
          if (*gen > current_gen) {
            ...
            if ((static_cast<std::size_t>(source_bs.shard_id) <
                 bucket_status.shards_done_with_gen.size()) &&
                !bucket_status.shards_done_with_gen[source_bs.shard_id]) {
              // use the error repo and sync status timestamp from the datalog
              // shard corresponding to source_bs
              error_repo = datalog_oid_for_error_repo(sc, sc->env->driver, pool,
                                                      source_bs);
              yield call(rgw::error_repo::write_cr(
                  sc->env->driver->getRados()->get_rados_handle(), error_repo,
                  rgw::error_repo::encode_key(source_bs, current_gen),
                  ceph::real_clock::zero()));
              if (retcode < 0) {
                tn->log(0, SSTR("ERROR: failed to log prev gen entry (bucket="
                                << source_bs.bucket << ", shard_id="
                                << source_bs.shard_id << ", gen=" << current_gen
                                << " in error repo: retcode=" << retcode));
              } else {
                tn->log(20, SSTR("logged prev gen entry (bucket="
                                 << source_bs.bucket
                                 << ", shard_id=" << source_bs.shard_id
                                 << ", gen=" << current_gen
                                 << " in error repo: retcode=" << retcode));
              }
            }
            ...
            tn->log(10, SSTR("ERROR: requested sync of future generation "
                             << *gen << " > " << current_gen << ", returning "
                             << retcode << " for later retry"));
            return set_cr_error(retcode);
        ...
        if (static_cast<std::size_t>(sync_pair.source_bs.shard_id) >=
            bucket_status.shards_done_with_gen.size()) {
          tn->log(1, SSTR("bucket shard " << sync_pair.source_bs
                                          << " index out of bounds"));
        ...
        yield call(new RGWSyncBucketShardCR(
            sc, data_lease_cr, sync_pair, sync_pipe, bucket_stopped,
            bucket_status.incremental_gen, tn, progress));
        if (retcode < 0) {
          tn->log(20,
                  SSTR("ERROR: incremental sync failed. error: " << retcode));
          return set_cr_error(retcode);
        }
      }
...
int RGWBucketPipeSyncStatusManager::do_init(const DoutPrefixProvider *dpp,
  ...
  sync_env.init(this, driver->ctx(), driver, driver->svc(), async_rados,
                &http_manager, error_logger.get(),
                driver->getRados()->get_sync_tracer(), sync_module, nullptr);

  sync_env.ostr = ostr;

  rgw_sync_pipe_info_set pipes;

  ret = cr_mgr.run(dpp, new RGWGetBucketPeersCR(
                            &sync_env, dest_bucket, source_zone, source_bucket,
                            &pipes, sync_env.sync_tracer->root_node));
  if (ret < 0) {
    ldpp_dout(this, 0) << "failed to get bucket source peers info: (ret=" << ret
  ...
  if (pipes.empty()) {
    ldpp_dout(this, 0)
        << "No peers. This is not a valid multisite configuration." << dendl;
    return -EINVAL;
  }
  ...
  for (auto &pipe : pipes) {
    ...
    if (!conn) {
      ldpp_dout(this, 0) << "connection object to zone " << szone
                         << " does not exist" << dendl;
      return -EINVAL;
    }
    ...
    sources.emplace_back(&sync_env, szone, conn, pipe.source.get_bucket_info(),
                         pipe.target.get_bucket(), pipe.handler, z->name);
  }
...
int RGWBucketPipeSyncStatusManager::remote_info(const DoutPrefixProvider *dpp,
  ...
  if (r < 0) {
    ldpp_dout(dpp, 0) << __PRETTY_FUNCTION__ << ":" << __LINE__
                      << " rgw_read_remote_bilog_info: r=" << r << dendl;
    return r;
  }
  if (oldest_gen)
...
RGWBucketPipeSyncStatusManager::construct(
    const DoutPrefixProvider *dpp, rgw::sal::RadosStore *driver,
    std::optional<rgw_zone_id> source_zone,
    std::optional<rgw_bucket> source_bucket, const rgw_bucket &dest_bucket,
    std::ostream *ostr) {
...
int RGWBucketPipeSyncStatusManager::init_sync_status(
  ...
  for (auto &source : sources) {
    ...
    stack->call(new RGWSimpleRadosWriteCR<rgw_bucket_sync_status>(
        ...
        {sync_env.svc->zone->get_zone_params().log_pool,
         full_status_oid(source.sc.source_zone, source.info.bucket,
                         source.dest)},
...
RGWBucketPipeSyncStatusManager::read_sync_status(
  ...
  if (source_zone) {
    sz = std::find_if(sources.begin(), sources.end(), [this](const source &s) {
  ...
  if (ret < 0) {
    ldpp_dout(this, 5) << "Unable to get remote info: " << ret << dendl;
  ...
  for (auto shard = 0u; shard < num_shards; ++shard) {
    ...
    stack->call(new RGWReadBucketPipeSyncStatusCoroutine(
        &sz->sc, pair, &sync_status[shard], nullptr, latest_gen));
  }
  ...
  int operate(const DoutPrefixProvider *dpp) override {
    reenter(this) {
      ...
      for (retries = 0u, retcode = -EDOM;
           (retries < allowed_retries) && (retcode != 0); ++retries) {
        ...
        if (retcode == -ECANCELED) {
          ldpp_dout(dpp, -1)
              << "ERROR: Got -ECANCELED for " << pair.source_bs << dendl;
          drain_all();
          return set_cr_error(retcode);
        } else if (retcode < 0) {
          ldpp_dout(dpp, 5)
              << "WARNING: Got error, retcode=" << retcode << " for "
              << pair.source_bs << "on retry " << retries + 1 << " of "
              << allowed_retries << " allowed" << dendl;
      ...
      if (retcode < 0) {
        ldpp_dout(dpp, -1) << "ERROR: Exhausted retries for " << pair.source_bs
...
public:
  GenCR(RGWDataSyncCtx &sc, const rgw_bucket &source, const rgw_bucket &dest,
      ...
      : RGWShardCollectCR(sc.cct, MAX_CONCURRENT_SHARDS), sc(sc), gen(gen) {
    pairs.resize(shards);
  ...
  int handle_result(int r) override {
    if (r < 0) {
      ldpp_dout(sc.env->dpp, 4)
          << "ERROR: Error syncing shard: " << cpp_strerror(r) << dendl;
    }
    return r;
  }
};
...
class SourceCR : public RGWCoroutine {
  ...
  const RGWBucketSyncFlowManager::pipe_handler &handler;
  const rgw_raw_obj status_obj{sc.env->svc->zone->get_zone_params().log_pool,
                               RGWBucketPipeSyncStatusManager::full_status_oid(
                                   sc.source_zone, info.bucket, dest)};
    ...
    reenter(this) {
      ...
      if (retcode < 0) {
        ldpp_dout(dpp, -1) << "ERROR: Unable to fetch status for zone="
                           << sc.source_zone << " retcode=" << retcode << dendl;
        drain_all();
        return set_cr_error(retcode);
      }
      ...
      do {
        ...
        if (state != BucketSyncState::Incremental) {
          pretty_print(
              sc.env, "Beginning full sync of bucket {} from source zone {}.\n",
              dest.name, zone_name);
          ldpp_dout(dpp, 5) << "SourceCR: Calling GenCR with "
                            << "gen=" << gen << ", num_shards=" << 1 << dendl;
          yield call(new GenCR(sc, info.bucket, dest, gen, 1, handler));
        } else {
          pretty_print(sc.env,
                       "Beginning incremental sync of bucket {}, generation {} "
                       "from source zone {}.\n",
                       dest.name, gen, zone_name);
          ldpp_dout(dpp, 5)
              << "SourceCR: Calling GenCR with "
              << "gen=" << gen << ", num_shards=" << num_shards << dendl;
          yield call(
              new GenCR(sc, info.bucket, dest, gen, num_shards, handler));
        }
        if (retcode < 0) {
          ldpp_dout(dpp, -1)
              << "ERROR: Giving up syncing from " << sc.source_zone
              << " retcode=" << retcode << dendl;
          drain_all();
          return set_cr_error(retcode);
        }
        ...
        if (retcode < 0) {
          ldpp_dout(dpp, -1)
              << "ERROR: Unable to fetch status for zone=" << sc.source_zone
              << " retcode=" << retcode << dendl;
          drain_all();
          return set_cr_error(retcode);
        }
        ...
        ldpp_dout(dpp, 5) << "SourceCR: "
                          ...
                          << status.incremental_gen
                          << ", status.shards_done_with_gen.size()="
                          << status.shards_done_with_gen.size() << dendl;
...
int RGWBucketPipeSyncStatusManager::run(const DoutPrefixProvider *dpp) {
  ...
  for (auto &source : sources) {
    ...
    stack->call(new rgw::bucket_sync_run::SourceCR(
        source.sc, source.info, source.dest, source.handler, source.zone_name));
    stacks.push_back(stack);
  }
...
std::ostream &
RGWBucketPipeSyncStatusManager::gen_prefix(std::ostream &out) const {
...
string
RGWBucketPipeSyncStatusManager::full_status_oid(const rgw_zone_id &source_zone,
...
string RGWBucketPipeSyncStatusManager::inc_status_oid(
    const rgw_zone_id &source_zone, const rgw_bucket_sync_pair_info &sync_pair,
    uint64_t gen) {
  if (sync_pair.source_bs.bucket == sync_pair.dest_bucket) {
    return bucket_status_oid_prefix + "." + source_zone.id + ":" +
           sync_pair.source_bs.get_key() + generation_token(gen);
  } else {
    return bucket_status_oid_prefix + "." + source_zone.id + ":" +
           sync_pair.dest_bucket.get_key() + ":" +
           sync_pair.source_bs.get_key() + generation_token(gen);
  }
}

string RGWBucketPipeSyncStatusManager::obj_status_oid(
    const rgw_bucket_sync_pipe &sync_pipe, const rgw_zone_id &source_zone,
    const rgw_obj &obj) {
  string prefix = object_status_oid_prefix + "." + source_zone.id + ":" +
                  obj.bucket.get_key();
...
int rgw_read_remote_bilog_info(const DoutPrefixProvider *dpp, RGWRESTConn *conn,
                               const rgw_bucket &bucket,
                               rgw_bucket_index_marker_info &info,
  ...
  if (r < 0) {
    ldpp_dout(dpp, -1) << "failed to fetch remote log markers: "
                       << cpp_strerror(r) << dendl;
    return r;
  }
  // parse shard markers
...
public:
  RGWCollectBucketSyncStatusCR(rgw::sal::RadosStore *driver, RGWDataSyncCtx *sc,
                               const rgw_bucket_sync_pair_info &sync_pair,
                               uint64_t gen, Vector *status)
      : RGWShardCollectCR(sc->cct, max_concurrent_shards), driver(driver),
        sc(sc), env(sc->env), gen(gen), sync_pair(sync_pair),
  ...
  bool spawn_next() override {
    ...
    spawn(new RGWReadBucketPipeSyncStatusCoroutine(sc, sync_pair, &*i, nullptr,
                                                   gen),
          false);
    ++i;
...
int rgw_read_bucket_full_sync_status(const DoutPrefixProvider *dpp,
  ...
  try {
    ...
    *status = result;
    ...
    lderr(svc->ctx()) << "error decoding " << obj << ": " << err.what()
                      << dendl;
    return -EIO;
  }
}

int rgw_read_bucket_inc_sync_status(
    const DoutPrefixProvider *dpp, rgw::sal::RadosStore *driver,
    const rgw_sync_bucket_pipe &pipe, uint64_t gen,
    std::vector<rgw_bucket_shard_sync_info> *status) {
  if (!pipe.source.zone || !pipe.source.bucket || !pipe.dest.zone ||
      !pipe.dest.bucket) {
    return -EINVAL;
  }
  ...
  env.init(dpp, driver->ctx(), driver, driver->svc(),
           driver->svc()->async_processor, nullptr, nullptr, nullptr, module,
           nullptr);
  ...
  RGWCoroutinesManager crs(driver->ctx(),
                           driver->getRados()->get_cr_registry());
  return crs.run(dpp, new RGWCollectBucketSyncStatusCR(driver, &sc, sync_pair,
                                                       gen, status));
}

void rgw_data_sync_info::generate_test_instances(
...
void rgw_data_sync_marker::generate_test_instances(
...
void rgw_data_sync_status::generate_test_instances(
