...
int RGWHTTPSimpleRequest::handle_header(const string &name, const string &val) {
  if (name == "CONTENT_LENGTH") {
    ...
    if (!err.empty()) {
      ldpp_dout(this, 0) << "ERROR: failed converting content length (" << val
                         << ") to int " << dendl;
      return -EINVAL;
    }
...
int RGWHTTPSimpleRequest::receive_data(void *ptr, size_t len, bool *pause) {
  ...
  left_len =
      max_response > response.length() ? (max_response - response.length()) : 0;
  if (left_len == 0)
...
static void do_get_params_str(const param_vec_t &params,
                              map<string, string> &extra_args, string &dest) {
...
void RGWHTTPSimpleRequest::get_params_str(map<string, string> &extra_args,
                                          string &dest) {
...
static int sign_request_v2(const DoutPrefixProvider *dpp,
                           const RGWAccessKey &key, const string &region,
                           const string &service, RGWEnv &env, req_info &info,
  ...
  if (cct->_conf->subsys.should_gather<ceph_subsys_rgw, 20>()) {
    for (const auto &i : env.get_map()) {
      ldpp_dout(dpp, 20) << __func__ << "():> " << i.first << " -> "
                         << rgw::crypt_sanitize::x_meta_map{i.first, i.second}
                         << dendl;
    }
  }

  string canonical_header;
  if (!rgw_create_s3_canonical_header(dpp, info, NULL, canonical_header,
                                      false)) {
    ldpp_dout(dpp, 0) << "failed to create canonical s3 header" << dendl;
    return -EINVAL;
  }

  ldpp_dout(dpp, 10) << "generated canonical header: " << canonical_header
                     << dendl;
...
static int sign_request_v4(const DoutPrefixProvider *dpp,
  ...
  // Simplified logging - only essential signature info
  ldpp_dout(dpp, 0) << "[CLIENT] Signing request: " << info.method << " " 
                    << info.request_uri << dendl;
  
  // Only log key-marker if present
  const auto &args_map = info.args.get_params();
  auto key_marker = args_map.find("key-marker");
  if (key_marker != args_map.end()) {
    ldpp_dout(dpp, 0) << "[CLIENT] key-marker=" << key_marker->second << dendl;
  }
  ...
  if (service == "s3") {
    sigv4_data = rgw::auth::s3::AWSSignerV4::prepare(
        dpp, key.id, region, service, info, opt_content, true);
  } else {
    sigv4_data = rgw::auth::s3::AWSSignerV4::prepare(
        dpp, key.id, region, service, info, opt_content, false);
  }

  // Log only essential signature components
  ldpp_dout(dpp, 0) << "[CLIENT] string_to_sign:\n" << sigv4_data.string_to_sign << dendl;

  auto sigv4_headers = sigv4_data.signature_factory(dpp, key.key, sigv4_data);

  // Extract and log only the signature from Authorization header
  for (auto &entry : sigv4_headers) {
    if (entry.first == "Authorization") {
      size_t sig_pos = entry.second.find("Signature=");
      if (sig_pos != string::npos) {
        string signature = entry.second.substr(sig_pos + 10);
        ldpp_dout(dpp, 0) << "[CLIENT] signature=" << signature << dendl;
      }
    }
...
static int sign_request(const DoutPrefixProvider *dpp, const RGWAccessKey &key,
                        ...
                        const bufferlist *opt_content) {
  auto authv =
      dpp->get_cct()->_conf.get_val<int64_t>("rgw_s3_client_max_sig_ver");
  if (authv > 0 && authv <= 3) {
...
static bool identify_scope(const DoutPrefixProvider *dpp, CephContext *cct,
                           const string &host, string *region,
                           string &service) {
  if (!boost::algorithm::ends_with(host, "amazonaws.com")) {
    ldpp_dout(dpp, 20) << "NOTICE: cannot identify region for connection to: "
                       << host << dendl;
    return false;
  }
  ...
  for (auto iter = vec.begin(); iter != vec.end(); ++iter) {
    ...
    if (s == "s3" || s == "execute-api" || s == "iam") {
      ...
      if (iter == vec.end()) {
        ldpp_dout(dpp, 0)
            << "WARNING: cannot identify region name from host name: " << host
            << dendl;
        return false;
      }
      auto &next = *iter;
...
static void scope_from_api_name(const DoutPrefixProvider *dpp, CephContext *cct,
                                const string &host,
                                std::optional<string> api_name, string *region,
                                string &service) {
...
int RGWRESTSimpleRequest::forward_request(const DoutPrefixProvider *dpp,
  ...
  size_t pos =
      new_info.request_uri.substr(1, new_info.request_uri.size() - 1).find("/");
  string bucket = new_info.request_uri.substr(1, pos);
  url_encode(bucket, bucket_encode);
  if (std::string::npos != pos)
    request_uri_encode =
        string("/") + bucket_encode + new_info.request_uri.substr(pos + 1);
  else
...
public:
  ...
  int handle_data(bufferlist &bl, off_t bl_ofs, off_t bl_len)
      override; /* callback for object iteration when sending data */
};

int RGWRESTStreamOutCB::handle_data(bufferlist &bl, off_t bl_ofs,
                                    off_t bl_len) {
  dout(20) << "RGWRESTStreamOutCB::handle_data bl.length()=" << bl.length()
...
RGWRESTStreamS3PutObj::~RGWRESTStreamS3PutObj() { delete out_cb; }

static void grants_by_type_add_one_grant(map<int, string> &grants_by_type,
                                         int perm, const ACLGrant &grant) {
...
static bool grants_by_type_check_perm(map<int, string> &grants_by_type,
                                      int perm, const ACLGrant &grant,
                                      int check_perm) {
...
static void grants_by_type_add_perm(map<int, string> &grants_by_type, int perm,
                                    const ACLGrant &grant) {
...
static void add_grants_headers(map<int, string> &grants, RGWEnv &env,
                               meta_map_t &meta_map) {
  struct grant_type_to_header *t;
...
RGWRESTGenerateHTTPHeaders::RGWRESTGenerateHTTPHeaders(CephContext *_cct,
                                                       RGWEnv *_env,
                                                       req_info *_info)
    : DoutPrefix(_cct, dout_subsys, "rest gen http headers: "), cct(_cct),
      new_env(_env), new_info(_info) {}

void RGWRESTGenerateHTTPHeaders::init(const string &_method, const string &host,
                                      const string &resource_prefix,
                                      const string &_url,
                                      const string &resource,
                                      const param_vec_t &params,
...
void RGWRESTGenerateHTTPHeaders::set_extra_headers(
...
int RGWRESTGenerateHTTPHeaders::set_obj_attrs(
    const DoutPrefixProvider *dpp, map<string, bufferlist> &rgw_attrs) {
...
void RGWRESTGenerateHTTPHeaders::set_http_attrs(
  ...
  for (auto &attr : http_attrs) {
    ...
    if (is_x_amz(name)) {
      ...
      new_env->set(
          attr.first,
          val); /* Ugh, using the uppercase representation,
...
void RGWRESTGenerateHTTPHeaders::set_policy(
...
int RGWRESTGenerateHTTPHeaders::sign(const DoutPrefixProvider *dpp,
                                     ...
                                     const bufferlist *opt_content) {
  int ret =
      sign_request(dpp, key, region, service, *new_env, *new_info, opt_content);
  if (ret < 0) {
    ldpp_dout(dpp, 0) << "ERROR: failed to sign request" << dendl;
    return ret;
  }
...
void RGWRESTStreamS3PutObj::put_obj_init(const DoutPrefixProvider *dpp,
                                         RGWAccessKey &key, const rgw_obj &obj,
                                         map<string, bufferlist> &attrs) {
  send_init(obj);
  send_ready(dpp, key, attrs);
}

void set_str_from_headers(map<string, string> &out_headers,
                          const string &header_name, string &str) {
...
static int parse_rgwx_mtime(const DoutPrefixProvider *dpp, CephContext *cct,
                            const string &s, ceph::real_time *rt) {
  string err;
  vector<string> vec;
  ...
  long nsecs = 0;
  if (!err.empty()) {
    ldpp_dout(dpp, 0) << "ERROR: failed converting mtime (" << s
                      << ") to real_time " << dendl;
    return -EINVAL;
  }
...
static void send_prepare_convert(const rgw_obj &obj, string *resource) {
  ...
  // do not encode slash. It leads to 404 errors when fetching objects inside
  // folders.
...
int RGWRESTStreamRWRequest::do_send_prepare(const DoutPrefixProvider *dpp,
                                            RGWAccessKey *key,
                                            map<string, string> &extra_headers,
                                            const string &resource,
                                            bufferlist *send_data) {
  ...
  headers_gen->init(method, host, resource_prefix, new_url, new_resource,
                    params, api_name);
...
int RGWRESTStreamRWRequest::send(RGWHTTPManager *mgr) {
  if (!headers_gen) {
    ldpp_dout(this, 0) << "ERROR: " << __func__
                       << "(): send_prepare() was not called: likey a bug!"
                       << dendl;
    return -EINVAL;
  }
...
int RGWHTTPStreamRWRequest::complete_request(optional_yield y, string *etag,
                                             real_time *mtime, uint64_t *psize,
  ...
  if (status >= 0) {
    ...
    if (psize) {
      ...
      *psize = strict_strtoll(size_str.c_str(), 10, &err);
      if (!err.empty()) {
        ldpp_dout(this, 0)
            << "ERROR: failed parsing embedded metadata object size ("
            << size_str << ") to int " << dendl;
        return -EIO;
      }
    }
  }

  for (auto iter = out_headers.begin(); pattrs && iter != out_headers.end();
       ++iter) {
    const string &attr_name = iter->first;
    if (attr_name.compare(0, sizeof(RGW_HTTP_RGWX_ATTR_PREFIX) - 1,
                          RGW_HTTP_RGWX_ATTR_PREFIX) == 0) {
...
int RGWHTTPStreamRWRequest::handle_header(const string &name,
                                          const string &val) {
  if (name == "RGWX_EMBEDDED_METADATA_LEN") {
    string err;
    long len = strict_strtol(val.c_str(), 10, &err);
    if (!err.empty()) {
      ldpp_dout(this, 0) << "ERROR: failed converting embedded metadata len ("
                         << val << ") to int " << dendl;
      return -EINVAL;
    }
...
int RGWHTTPStreamRWRequest::send_data(void *ptr, size_t len, bool *pause) {
  ...
  uint64_t send_size;
  {
    ...
    if (outbl.length() == 0) {
      if ((stream_writes && !write_stream_complete) || (write_ofs < send_len)) {
        *pause = true;
      }
      return 0;
    }
  ...
  /* don't need to be under write_lock here, avoid deadlocks in case notify
   * callback needs to lock */
  if (write_drain_cb) {
