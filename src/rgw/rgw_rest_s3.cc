...
void list_all_buckets_end(req_state *s) { s->formatter->close_section(); }
...
get_s3_expiration_header(req_state *s, const ceph::real_time &mtime) {
  return rgw::lc::s3_expiration_header(s, s->object->get_key(), s->tagset,
                                       mtime, s->bucket_attrs);
}

static inline bool get_s3_multipart_abort_header(req_state *s,
                                                 const ceph::real_time &mtime,
                                                 ceph::real_time &date,
                                                 std::string &rule_id) {
  return rgw::lc::s3_multipart_abort_header(s, s->object->get_key(), mtime,
                                            s->bucket_attrs, date, rule_id);
}
...
int get_encryption_defaults(req_state *s) {
  ...
  constexpr auto bucket_key_attr =
  ...
  if (aiter != buck_attrs.end()) {
    ...
    try {
      ...
      ldpp_dout(s, 5) << __func__ << "decode bucket_encryption_conf failed"
                      << dendl;
      return -EINVAL;
    }
  }
  if (meta_sse_group & SSE_C_GROUP) {
    ldpp_dout(s, 20)
        << "get_encryption_defaults: no defaults cause sse-c forced" << dendl;
  ...
  if (bucket_configuration_found) {
    ldpp_dout(s, 5) << "RGW_ATTR_BUCKET_ENCRYPTION ALGO: " << sse_algorithm
                    << dendl;
  }

  auto iter = s->info.crypt_attribute_map.find(encrypt_attr);
  if (iter != s->info.crypt_attribute_map.end()) {
    ldpp_dout(s, 20) << "get_encryption_defaults: found encrypt_attr "
                     << encrypt_attr << " = " << iter->second
                     << ", setting sse_algorithm to that" << dendl;
    rest_only = true;
    sse_algorithm = iter->second;
  } else if (sse_algorithm != "") {
    rgw_set_amz_meta_header(s->info.crypt_attribute_map, encrypt_attr,
                            sse_algorithm, OVERWRITE);
  }

  iter = s->info.crypt_attribute_map.find(kms_attr);
  if (iter != s->info.crypt_attribute_map.end()) {
    ldpp_dout(s, 20) << "get_encryption_defaults: found kms_attr " << kms_attr
                     << " = " << iter->second << ", setting kms_attr_seen"
                     << dendl;
    if (!rest_only) {
      s->err.message = std::string("incomplete rest sse parms: ") + kms_attr +
                       " not valid without kms";
      ldpp_dout(s, 5) << __func__ << "argument problem: " << s->err.message
                      << dendl;
      return -EINVAL;
    }
    kms_attr_seen = true;
    ...
    ldpp_dout(s, 20)
        << "get_encryption_defaults: no kms_attr, but kms_master_key_id = "
        << kms_master_key_id << ", setting kms_attr_seen" << dendl;
    kms_attr_seen = true;
    rgw_set_amz_meta_header(s->info.crypt_attribute_map, kms_attr,
                            kms_master_key_id, OVERWRITE);
  }

  iter = s->info.crypt_attribute_map.find(bucket_key_attr);
  if (iter != s->info.crypt_attribute_map.end()) {
    ldpp_dout(s, 20) << "get_encryption_defaults: found bucket_key_attr "
                     << bucket_key_attr << " = " << iter->second
                     << ", setting kms_attr_seen" << dendl;
    if (!rest_only) {
      s->err.message = std::string("incomplete rest sse parms: ") +
                       bucket_key_attr + " not valid without kms";
      ldpp_dout(s, 5) << __func__ << "argument problem: " << s->err.message
                      << dendl;
      return -EINVAL;
    }
    ...
    ldpp_dout(s, 20) << "get_encryption_defaults: no bucket_key_attr, but "
                        "bucket_key_enabled,  setting kms_attr_seen"
                     << dendl;
    kms_attr_seen = true;
    rgw_set_amz_meta_header(s->info.crypt_attribute_map, bucket_key_attr,
                            "true", OVERWRITE);
  }

  iter = s->info.crypt_attribute_map.find(context_attr);
  if (iter != s->info.crypt_attribute_map.end()) {
    ldpp_dout(s, 20) << "get_encryption_defaults: found context_attr "
                     << context_attr << " = " << iter->second
                     << ", setting kms_attr_seen" << dendl;
    if (!rest_only) {
      s->err.message = std::string("incomplete rest sse parms: ") +
                       context_attr + " not valid without kms";
      ldpp_dout(s, 5) << __func__ << "argument problem: " << s->err.message
                      << dendl;
      return -EINVAL;
    }
  ...
  if (kms_attr_seen && sse_algorithm == "") {
    ldpp_dout(s, 20) << "get_encryption_defaults: kms_attr but no algorithm, "
                        "defaulting to aws_kms"
                     << dendl;
    sse_algorithm = "aws:kms";
  }
  for (const auto &kv : s->info.crypt_attribute_map) {
    ldpp_dout(s, 20) << "get_encryption_defaults:  final map: " << kv.first
                     << " = " << kv.second << dendl;
  }
  ldpp_dout(s, 20) << "get_encryption_defaults:  kms_attr_seen is "
                   << kms_attr_seen << " and sse_algorithm is " << sse_algorithm
                   << dendl;
  if (kms_attr_seen && sse_algorithm != "aws:kms") {
    s->err.message =
        "algorithm <" + sse_algorithm + "> but got sse-kms attributes";
    return -EINVAL;
  }
...
int RGWGetObj_ObjStore_S3Website::send_response_data(bufferlist &bl,
                                                     off_t bl_ofs,
                                                     off_t bl_len) {
  ...
  if (iter != attrs.end()) {
    ...
    s->redirect = bl.c_str();
    s->err.http_ret = 301;
    ldpp_dout(this, 20) << __CEPH_ASSERT_FUNCTION
                        << " redirecting per x-amz-website-redirect-location="
                        << s->redirect << dendl;
...
int RGWGetObj_ObjStore_S3::get_params(optional_yield y) {
  ...
  sync_cloudtiered =
      s->info.args.exists(RGW_SYS_PARAM_PREFIX "sync-cloudtiered");

  dst_zone_trace =
      s->info.args.get(RGW_SYS_PARAM_PREFIX "if-not-replicated-to");
...
template <class T>
int decode_attr_bl_single_value(map<string, bufferlist> &attrs,
...
int RGWGetObj_ObjStore_S3::send_response_data(bufferlist &bl, off_t bl_ofs,
  ...
  if (s->system_request &&
    ...
    if (rgwx_stat) {
      /*
       * in this case, we're not returning the object's content, only the
       * prepended extra metadata
       */
      total_len = 0;
    }
  ...
  if (s->system_request && !real_clock::is_zero(lastmod)) {
    ...
    r = decode_attr_bl_single_value(attrs, RGW_ATTR_SOURCE_ZONE,
                                    &source_zone_short_id, (uint32_t)0);
    if (r < 0) {
      ldpp_dout(this, 0) << "ERROR: failed to decode pg ver attr, ignoring"
                         << dendl;
    }
  ...
  // replication status
  if (auto i = attrs.find(RGW_ATTR_OBJ_REPLICATION_STATUS); i != attrs.end()) {
    dump_header(s, "x-amz-replication-status", i->second);
  }
  if (auto i = attrs.find(RGW_ATTR_OBJ_REPLICATION_TRACE); i != attrs.end()) {
    try {
    ...
    } // omit x-rgw-replicated-from headers
  }
  ...
  if (!op_ret) {
    ...
    for (auto iter = attrs.begin(); iter != attrs.end(); ++iter) {
      ...
      if (aiter != rgw_to_http_attrs.end()) {
      ...
      } else if (iter->first.compare(RGW_ATTR_OBJECT_RETENTION) == 0 &&
                 get_retention) {
        ...
        try {
          ...
          ldpp_dout(this, 0)
              << "ERROR: failed to decode RGWObjectRetention" << dendl;
        }
      } else if (iter->first.compare(RGW_ATTR_OBJECT_LEGAL_HOLD) == 0 &&
                 get_legal_hold) {
        RGWObjectLegalHold legal_hold;
        try {
          decode(legal_hold, iter->second);
          dump_header(s, "x-amz-object-lock-legal-hold",
                      legal_hold.get_status());
        } catch (buffer::error &err) {
          ldpp_dout(this, 0)
              << "ERROR: failed to decode RGWObjectLegalHold" << dendl;
        }
      }
    }
  }
...
int RGWGetObj_ObjStore_S3::get_decrypt_filter(
    std::unique_ptr<RGWGetObj_Filter> *filter, RGWGetObj_Filter *cb,
  ...
  *filter = std::make_unique<RGWGetObj_BlockDecrypt>(
      s, s->cct, cb, std::move(block_crypt), std::move(parts_len), s->yield);
  return 0;
}
int RGWGetObj_ObjStore_S3::verify_requester(
  ...
  if (!s->user->get_caps().check_cap("amz-cache", RGW_CAP_READ) && !ret &&
      s->info.env->exists("HTTP_X_AMZ_CACHE"))
    ret = override_range_hdr(auth_registry, y);
  return ret;
}

int RGWGetObj_ObjStore_S3::override_range_hdr(
    const rgw::auth::StrategyRegistry &auth_registry, optional_yield y) {
  int ret = -EINVAL;
  ...
  for (std::string_view hdr : ceph::split(cache_hdr, hdrs_split)) {
    ...
    ldpp_dout(this, 10) << "after splitting cache kv key: " << key << " " << *v
                        << dendl;
...
int RGWPutBucketTags_ObjStore_S3::get_params(const DoutPrefixProvider *dpp,
                                             optional_yield y) {
  RGWXMLParser parser;
  ...
  RGWObjTags obj_tags(
      50); // A tag set can contain as many as 50 tags, or it can be empty.
  r = tagging.rebuild(obj_tags);
  if (r < 0)
    return r;
...
struct ReplicationConfiguration {
  ...
  struct Rule {
    struct DeleteMarkerReplication {
      ...
      void dump_xml(Formatter *f) const { encode_xml("Status", status, f); }

      bool is_valid(CephContext *cct) const {
        bool result = is_valid_status(status);
        if (!result) {
          ldout(cct, 5) << "NOTICE: bad status provided in "
                           "DeleteMarkerReplication element (status="
                        << status << ")" << dendl;
        }
        return result;
      }
    ...
    struct Source { /* rgw extension */
      ...
      void dump_xml(Formatter *f) const { encode_xml("Zone", zone_names, f); }
    };
    ...
    struct Destination {
      struct AccessControlTranslation {
        ...
        void dump_xml(Formatter *f) const { encode_xml("Owner", owner, f); }
      };
      ...
      void decode_xml(XMLObj *obj) {
        RGWXMLDecoder::decode_xml("AccessControlTranslation", acl_translation,
                                  obj);
    ...
    struct Filter {
      ...
      struct AndElements {
        ...
        bool empty() const { return !prefix && (tags.size() == 0); }
      ...
      bool empty() const { return (!prefix && !tag && !and_elements); }
      ...
      bool is_valid(CephContext *cct) const {
        if (tag && prefix) {
          ldout(cct, 5) << "NOTICE: both tag and prefix were provided in "
        ...
        if (and_elements) {
          if (prefix && and_elements->prefix) {
            ldout(cct, 5) << "NOTICE: too many prefixes were provided in re"
                          << dendl;
            return false;
          }
        }
      ...
      int to_sync_pipe_filter(CephContext *cct, rgw_sync_pipe_filter *f) const {
        if (!is_valid(cct)) {
          return -EINVAL;
        }
        if (prefix) {
    ...
    set<rgw_zone_id>
    get_zone_ids_from_names(rgw::sal::Driver *driver,
      ...
      for (auto &name : zone_names) {
        ...
        int ret =
            driver->get_zone()->get_zonegroup().get_zone_by_name(name, &zone);
        if (ret >= 0) {
    ...
    vector<string>
    get_zone_names_from_ids(rgw::sal::Driver *driver,
      ...
      for (auto &id : zone_ids) {
        std::unique_ptr<rgw::sal::Zone> zone;
        int ret =
            driver->get_zone()->get_zonegroup().get_zone_by_id(id.id, &zone);
        if (ret >= 0) {
    ...
    void decode_xml(XMLObj *obj) {
      RGWXMLDecoder::decode_xml("DeleteMarkerReplication",
                                delete_marker_replication, obj);
      ...
      if (!filter) {
        ...
        /* don't want to have filter reset because it might have been
         * initialized when decoding prefix
         */
        RGWXMLDecoder::decode_xml("Filter", *filter, obj);
      }
    ...
    bool is_valid(CephContext *cct) const {
      if (!is_valid_status(status)) {
        ldout(cct, 5) << "NOTICE: bad status provided in rule (status="
                      << status << ")" << dendl;
        return false;
      }
      if ((filter && !filter->is_valid(cct)) ||
          (delete_marker_replication &&
           !delete_marker_replication->is_valid(cct))) {
        return false;
      }
      return true;
    }
    ...
    int to_sync_policy_pipe(req_state *s, rgw::sal::Driver *driver,
                            rgw_sync_bucket_pipes *pipe, bool *enabled) const {
      if (!is_valid(s->cct)) {
        return -EINVAL;
      }
      ...
      rgw_bucket_key dest_bk(user_id.tenant, destination.bucket);

      if (source && !source->zone_names.empty()) {
        pipe->source.zones =
            get_zone_ids_from_names(driver, source->zone_names);
      } else {
        pipe->source.set_all_zones(true);
      }
      if (!destination.zone_names.empty()) {
        pipe->dest.zones =
            get_zone_ids_from_names(driver, destination.zone_names);
      } else {
      ...
      if (filter) {
        int r =
            filter->to_sync_pipe_filter(s->cct, &pipe->params.source.filter);
        if (r < 0) {
          return r;
        }
      }
      if (destination.acl_translation) {
        ...
        u.from_str(destination.acl_translation
                       ->owner); /* explicit tenant will override tenant,
    ...
    void from_sync_policy_pipe(rgw::sal::Driver *driver,
      ...
      if (pipe.source.all_zones) {
        ...
        source.emplace();
        source->zone_names =
            get_zone_names_from_ids(driver, *pipe.source.zones);
      }

      if (!pipe.dest.all_zones && pipe.dest.zones) {
        destination.zone_names =
            get_zone_names_from_ids(driver, *pipe.dest.zones);
      }

      if (pipe.params.dest.acl_translation) {
        destination.acl_translation.emplace();
        destination.acl_translation->owner =
            pipe.params.dest.acl_translation->owner.to_str();
      }
  ...
  int to_sync_policy_groups(req_state *s, rgw::sal::Driver *driver,
    ...
    disabled_group.status =
        rgw_sync_policy_group::Status::ALLOWED; /* not enabled, not forbidden */
    ...
    for (auto &rule : rules) {
      ...
      if (r < 0) {
        ldpp_dout(s, 5) << "NOTICE: failed to convert replication "
                           "configuration into sync policy pipe (rule.id="
                        << rule.id << "): " << cpp_strerror(-r) << dendl;
        return r;
      }
...
void RGWGetBucketReplication_ObjStore_S3::send_response_data() {
  ...
  if (!op_ret) {
    s->formatter->open_object_section_in_ns("ReplicationConfiguration",
                                            XMLNS_AWS_S3);
...
void RGWDeleteBucketReplication_ObjStore_S3::update_sync_policy(
...
void RGWListBuckets_ObjStore_S3::send_response_data(
    std::span<const RGWBucketEnt> buckets) {
  if (!sent_data)
    return;
...
static void dump_usage_bucket_info(Formatter *formatter,
                                   const std::string &name,
                                   const bucket_meta_entry &entry) {
  formatter->open_object_section("Entry");
...
void RGWGetUsage_ObjStore_S3::send_response() {
  ...
  for (iter = usage.begin(); iter != usage.end(); ++iter) {
    ...
    if (show_log_entries) {
      ...
      if (categories.empty() || categories.count("s3select")) {
        encode_json("BytesProcessed", entry.s3select_usage.bytes_processed,
                    formatter);
        encode_json("BytesReturned", entry.s3select_usage.bytes_returned,
                    formatter);
      }
  ...
  if (show_log_sum) {
    ...
    encode_json("QuotaMaxBytes", user_info.quota.user_quota.max_size,
                formatter);
    encode_json("QuotaMaxBuckets", user_info.max_buckets, formatter);
    encode_json("QuotaMaxObjCount", user_info.quota.user_quota.max_objects,
                formatter);
    encode_json("QuotaMaxBytesPerBucket",
                user_info.quota.bucket_quota.max_objects, formatter);
    encode_json("QuotaMaxObjCountPerBucket",
                user_info.quota.bucket_quota.max_size, formatter);
...
int RGWListBucket_ObjStore_S3::get_common_params() {
  ...
  if (s->system_request) {
    ...
    if (shard_id_str) {
      ...
      if (!err.empty()) {
        ldpp_dout(this, 5) << "bad shard id specified: " << shard_id_str
                           << dendl;
        return -EINVAL;
      }
    } else {
...
void RGWListBucket_ObjStore_S3::send_common_versioned_response() {
  ...
  s->formatter->dump_string("IsTruncated",
                            (max && is_truncated ? "true" : "false"));
  ...
  if (op_ret >= 0) {
    ...
    for (iter = objs.begin(); iter != objs.end(); ++iter) {
      const char *section_name =
          (iter->is_delete_marker() ? "DeleteMarker" : "Version");
      ...
      if (!iter->is_delete_marker()) {
        ...
        auto &storage_class = rgw_placement_rule::get_canonical_storage_class(
            iter->meta.storage_class);
...
void RGWListBucket_ObjStore_S3v2::send_versioned_response() {
  ...
  if (is_truncated && !next_marker.empty()) {
    ...
    s->formatter->dump_string("NextVersionIdContinuationToken",
                              next_marker.instance);
  }
  ...
  if (op_ret >= 0) {
    ...
    for (iter = objs.begin(); iter != objs.end(); ++iter) {
      const char *section_name =
          (iter->is_delete_marker() ? "DeleteContinuationToken" : "Version");
      s->formatter->open_object_section(section_name);
      if (objs_container) {
        s->formatter->dump_bool("IsDeleteContinuationToken",
                                iter->is_delete_marker());
      }
      rgw_obj_key key(iter->key);
      dump_urlsafe(s, encode_key, "Key", key.name);
      string version_id = key.instance;
...
void RGWGetBucketLocation_ObjStore_S3::send_response() {
  ...
  s->formatter->dump_format_ns("LocationConstraint", XMLNS_AWS_S3, "%s",
                               api_name.c_str());
  rgw_flush_formatter_and_reset(s, s->formatter);
}
...
int RGWSetBucketVersioning_ObjStore_S3::get_params(optional_yield y) {
  ...
  if (!RGWXMLDecoder::decode_xml("VersioningConfiguration", status_conf,
                                 &parser)) {
  ...
  if (status_conf.mfa_status != ver_config_status::MFA_UNKNOWN) {
    ...
    default:
      ldpp_dout(this, 0)
          << "ERROR: "
             "RGWSetBucketVersioning_ObjStore_S3::get_params(optional_yield "
             "y): unexpected switch case mfa_status="
          << status_conf.mfa_status << dendl;
      r = -EIO;
    }
...
int RGWSetBucketWebsite_ObjStore_S3::get_params(optional_yield y) {
  ...
  try {
    RGWXMLDecoder::decode_xml("WebsiteConfiguration", website_conf, &parser,
                              true);
  } catch (RGWXMLDecoder::err &err) {
  ...
  if (website_conf.is_redirect_all &&
      website_conf.redirect_all.hostname.empty()) {
    s->err.message = "A host name must be provided to redirect all requests "
                     "(e.g. \"example.com\").";
    ldpp_dout(this, 5) << s->err.message << dendl;
    return -EINVAL;
  } else if (!website_conf.is_redirect_all && !website_conf.is_set_index_doc) {
    s->err.message = "A value for IndexDocument Suffix must be provided if "
...
#define WEBSITE_ROUTING_RULES_MAX_NUM 50
  ...
  if (routing_rules_num > max_num) {
    ldpp_dout(this, 4) << "An website routing config can have up to " << max_num
                       << " rules, request website routing rules num: "
                       << routing_rules_num << dendl;
    s->err.message = std::to_string(routing_rules_num) +
                     " routing rules provided, the number of routing rules in "
                     "a website configuration is limited to " +
...
static void dump_bucket_metadata(req_state *s, rgw::sal::Bucket *bucket,
  ...
  dump_header(s, "X-RGW-Object-Count",
              static_cast<long long>(stats.num_objects));
  ...
  if (s->auth.identity->is_owner_of(bucket->get_owner())) {
    ...
    dump_header(s, "X-RGW-Quota-User-Size",
                static_cast<long long>(user_info.quota.user_quota.max_size));
    dump_header(s, "X-RGW-Quota-User-Objects",
                static_cast<long long>(user_info.quota.user_quota.max_objects));
    dump_header(s, "X-RGW-Quota-Max-Buckets",
                static_cast<long long>(user_info.max_buckets));
    dump_header(s, "X-RGW-Quota-Bucket-Size",
                static_cast<long long>(bucket_quota.max_size));
    dump_header(s, "X-RGW-Quota-Bucket-Objects",
                static_cast<long long>(bucket_quota.max_objects));
  }
}
...
static int create_s3_policy(req_state *s, rgw::sal::Driver *driver,
  ...
  return rgw::s3::create_canned_acl(owner, s->bucket_owner, s->canned_acl,
                                    policy);
}
...
class RGWCreateBucketParser : public RGWXMLParser {
  XMLObj *alloc_obj(const char *el) override { return new XMLObj; }
...
int RGWCreateBucket_ObjStore_S3::get_params(optional_yield y) {
  ...
  if (data.length()) {
    ...
    if (!parser.get_location_constraint(location_constraint)) {
      ldpp_dout(this, 0)
          << "provided input did not specify location constraint correctly"
          << dendl;
      return -EINVAL;
    }
  ...
  if (iter != s->info.x_meta_map.end()) {
    ...
    createparams.obj_lock_enabled =
        boost::algorithm::iequals(iter->second, "true");
  }
  return 0;
}
...
static inline void map_qs_metadata(req_state *s, bool crypto_too) {
  ...
  for (const auto &elt : params) {
    ...
    if (crypto_too &&
        k.find("x-amz-server-side-encryption") == /* offset */ 0) {
      rgw_set_amz_meta_header(s->info.crypt_attribute_map, k, elt.second,
                              OVERWRITE);
    }
  }
}
...
int RGWPutObj_ObjStore_S3::get_params(optional_yield y) {
  ...
  auto obj_lock_date_str =
      s->info.env->get("HTTP_X_AMZ_OBJECT_LOCK_RETAIN_UNTIL_DATE");
  auto obj_legal_hold_str =
      s->info.env->get("HTTP_X_AMZ_OBJECT_LOCK_LEGAL_HOLD");
  if (obj_lock_mode_str && obj_lock_date_str) {
    ...
    if (boost::none == date ||
      ...
      ret = -EINVAL;
      ldpp_dout(this, 0) << "invalid x-amz-object-lock-retain-until-date value"
                         << dendl;
      return ret;
    }
    if (strcmp(obj_lock_mode_str, "GOVERNANCE") != 0 &&
        strcmp(obj_lock_mode_str, "COMPLIANCE") != 0) {
      ret = -EINVAL;
    ...
    obj_retention = new RGWObjectRetention(obj_lock_mode_str, *date);
             ...
             (!obj_lock_mode_str && obj_lock_date_str)) {
    ret = -EINVAL;
    ldpp_dout(this, 0) << "need both x-amz-object-lock-mode and "
                          "x-amz-object-lock-retain-until-date "
                       << dendl;
    return ret;
  }
  if (obj_legal_hold_str) {
    if (strcmp(obj_legal_hold_str, "ON") != 0 &&
        strcmp(obj_legal_hold_str, "OFF") != 0) {
      ret = -EINVAL;
      ldpp_dout(this, 0) << "invalid x-amz-object-lock-legal-hold value"
                         << dendl;
      return ret;
    }
  ...
  if (!s->bucket->get_info().obj_lock_enabled() &&
      (obj_retention || obj_legal_hold)) {
    ldpp_dout(this, 0) << "ERROR: object retention or legal hold can't be set "
  ...
  if (!multipart_part_str.empty()) {
    ...
    if (!err.empty()) {
      ldpp_dout(s, 10) << "bad part number: " << multipart_part_str << ": "
                       << err << dendl;
      return -EINVAL;
    }
  ...
  if (append) {
    ...
    if (!err.empty()) {
      ...
      ldpp_dout(s, 10) << "bad position: " << pos_str << ": "
...
void RGWPutObj_ObjStore_S3::send_response() {
  if (op_ret) {
    ...
    if (s->cct->_conf->rgw_s3_success_create_obj_status) {
      op_ret =
          get_success_retcode(s->cct->_conf->rgw_s3_success_create_obj_status);
      set_req_state_err(s, op_ret);
    }
...
static inline void set_attr(map<string, bufferlist> &attrs, const char *key,
...
int RGWPutObj_ObjStore_S3::get_decrypt_filter(
    std::unique_ptr<RGWGetObj_Filter> *filter, RGWGetObj_Filter *cb,
    map<string, bufferlist> &attrs, bufferlist *manifest_bl) {
...
int RGWPutObj_ObjStore_S3::get_encrypt_filter(
  ...
  if (!multipart_upload_id.empty()) {
    ...
    if (res == 0) {
      ...
      res = rgw_s3_prepare_decrypt(s, s->yield, obj->get_attrs(), &block_crypt,
                                   crypt_http_responses);
      if (res == 0 && block_crypt != nullptr)
        filter->reset(new RGWPutObj_BlockEncrypt(
            s, s->cct, cb, std::move(block_crypt), s->yield));
    }
...
int RGWPostObj_ObjStore_S3::get_params(optional_yield y) {
  ...
  do {
    ...
    if (s->cct->_conf->subsys.should_gather<ceph_subsys_rgw, 20>()) {
      ldpp_dout(this, 20) << "read part header -- part.name=" << part.name
                          << dendl;
    ...
    if (stringcasecmp(part.name, "file") ==
        0) { /* beginning of data transfer */
...
int RGWPostObj_ObjStore_S3::get_tags() {
  ...
  if (part_str(parts, "tagging", &tags_str)) {
    ...
    if (!parser.init()) {
      ...
      return -EINVAL; // TODO: This class of errors in rgw code should be a 5XX
                      // error
    }
...
int RGWPostObj_ObjStore_S3::get_policy(optional_yield y) {
  if (part_bl(parts, "policy", &s->auth.s3_postobj_creds.encoded_policy)) {
    ...
    if ((part_str(parts, "x-amz-algorithm",
                  &s->auth.s3_postobj_creds.x_amz_algorithm)) &&
        (s->auth.s3_postobj_creds.x_amz_algorithm == AWS4_HMAC_SHA256_STR)) {
      ldpp_dout(this, 0)
          << "Signature verification algorithm AWS v4 (AWS4-HMAC-SHA256)"
          << dendl;
      aws4_auth = true;
    } else {
    ...
    if (part_str(parts, "x-amz-security-token",
    ...
    /* FIXME: this is a makeshift solution. The browser upload authentication
     * will be handled by an instance of rgw::auth::Completer spawned in
     * Handler's authorize() method. */
    const int ret = rgw::auth::Strategy::apply(
        this, auth_registry_ptr->get_s3_post(), s, y);
    if (ret != 0) {
      return -EACCES;
    } else {
  ...
  int r =
      rgw::s3::create_canned_acl(s->owner, s->bucket_owner, canned_acl, policy);
  if (r < 0) {
...
void RGWPostObj_ObjStore_S3::send_response() {
  ...
  if (op_ret == STATUS_CREATED) {
    ...
    if (!s->bucket_tenant.empty()) {
      s->formatter->dump_format("Location", "%s/%s:%s/%s", base_uri.c_str(),
                                url_encode(s->bucket_tenant).c_str(),
                                url_encode(s->bucket_name).c_str(),
                                url_encode(s->object->get_name()).c_str());
      s->formatter->dump_string("Tenant", s->bucket_tenant);
    } else {
      s->formatter->dump_format("Location", "%s/%s/%s", base_uri.c_str(),
                                url_encode(s->bucket_name).c_str(),
                                url_encode(s->object->get_name()).c_str());
    }
...
int RGWDeleteObj_ObjStore_S3::get_params(optional_yield y) {
  const char *if_unmod =
      s->info.env->get("HTTP_X_AMZ_DELETE_IF_UNMODIFIED_SINCE");

  if (s->system_request) {
    s->info.args.get_bool(RGW_SYS_PARAM_PREFIX "no-precondition-error",
  ...
  if (if_unmod) {
    ...
    if (utime_t::parse_date(if_unmod_decoded, &epoch, &nsec) < 0) {
      ldpp_dout(this, 10) << "failed to parse time: " << if_unmod_decoded
                          << dendl;
      return -EINVAL;
    }
  ...
  const char *bypass_gov_header =
      s->info.env->get("HTTP_X_AMZ_BYPASS_GOVERNANCE_RETENTION");
  if (bypass_gov_header) {
    std::string bypass_gov_decoded = url_decode(bypass_gov_header);
    bypass_governance_mode =
        boost::algorithm::iequals(bypass_gov_decoded, "true");
  }
...
int RGWCopyObj_ObjStore_S3::get_params(optional_yield y) {
  ...
  if (obj_lock_mode_str && obj_lock_date_str) {
    ...
    s->err.message = "need both x-amz-object-lock-mode and "
                     "x-amz-object-lock-retain-until-date ";
    ldpp_dout(this, 0) << s->err.message << dendl;
    return -EINVAL;
  }
  ...
  if (s->system_request) {
    ...
    s->info.args.get_bool(RGW_SYS_PARAM_PREFIX "copy-if-newer", &copy_if_newer,
                          false);
  }
  ...
  if (source_zone.empty() && (s->bucket->get_tenant() == s->src_tenant_name) &&
...
int RGWCopyObj_ObjStore_S3::check_storage_class(
    const rgw_placement_rule &src_placement) {
...
int RGWPutACLs_ObjStore_S3::get_params(optional_yield y) {
  ...
  if (ret >= 0) {
     ...
     * about that here) */
    if ((ret == -ERR_LENGTH_REQUIRED) && !!(s->length)) {
      return 0;
    }
  }
  return ret;
...
int RGWPutACLs_ObjStore_S3::get_policy_from_state(
    const ACLOwner &owner, RGWAccessControlPolicy &policy) {
...
int RGWPutCORS_ObjStore_S3::get_params(optional_yield y) {
  ...
  cors_config = static_cast<RGWCORSConfiguration_S3 *>(
...
#define CORS_RULES_MAX_NUM 100
  ...
  if (cors_rules_num > max_num) {
    ldpp_dout(this, 4) << "An cors config can have up to " << max_num
                       << " rules, request cors rules num: " << cors_rules_num
                       << dendl;
    s->err.message =
...
void RGWInitMultipart_ObjStore_S3::send_response() {
  ...
  string rule_id;
  bool exist_multipart_abort =
      get_s3_multipart_abort_header(s, mtime, abort_date, rule_id);
...
int RGWInitMultipart_ObjStore_S3::prepare_encryption(
  ...
  int res = 0;
  res =
      rgw_s3_prepare_encrypt(s, s->yield, attrs, nullptr, crypt_http_responses);
  return res;
}
...
void RGWCompleteMultipart_ObjStore_S3::send_response() {
  ...
  if (op_ret == 0) {
    ...
    std::string base_uri = compute_domain_uri(s);
    if (!s->bucket_tenant.empty()) {
      s->formatter->dump_format(
          "Location", "%s/%s:%s/%s", base_uri.c_str(), s->bucket_tenant.c_str(),
          s->bucket_name.c_str(), s->object->get_name().c_str());
      s->formatter->dump_string("Tenant", s->bucket_tenant);
    } else {
      s->formatter->dump_format("Location", "%s/%s/%s", base_uri.c_str(),
                                s->bucket_name.c_str(),
...
void RGWListMultipart_ObjStore_S3::send_response() {
  ...
  if (op_ret == 0) {
    ...
    map<uint32_t, std::unique_ptr<rgw::sal::MultipartPart>>::reverse_iterator
        test_iter;
    int cur_max = 0;
...
void RGWListBucketMultiparts_ObjStore_S3::send_response() {
  ...
  s->formatter->open_object_section_in_ns("ListMultipartUploadsResult",
                                          XMLNS_AWS_S3);
  if (!s->bucket_tenant.empty())
    s->formatter->dump_string("Tenant", s->bucket_tenant);
  s->formatter->dump_string("Bucket", s->bucket_name);
...
void RGWDeleteMultiObj_ObjStore_S3::send_partial_response(
    const rgw_obj_key &key, bool delete_marker, const string &marker_version_id,
    int ret, boost::asio::deadline_timer *formatter_flush_cond) {
  if (!key.empty()) {
...
void RGWGetBucketObjectLock_ObjStore_S3::send_response() {
  ...
  encode_xml("ObjectLockConfiguration", s->bucket->get_info().obj_lock,
             s->formatter);
  rgw_flush_formatter_and_reset(s, s->formatter);
}
...
RGWOp *RGWHandler_REST_Bucket_S3::get_obj_op(bool get_data) const {
  ...
  if (get_data) {
    ...
    default:
      ldpp_dout(s, 5) << __func__ << ": unsupported list-type " << list_type
                      << dendl;
      return new RGWListBucket_ObjStore_S3;
    }
  } else {
...
int RGWHandler_REST_S3::init_from_header(rgw::sal::Driver *driver, req_state *s,
  ...
  if (s->init_state.url_bucket.empty()) {
    ...
    if (!encoded_obj_str.empty()) {
      if (s->bucket) {
        s->object = s->bucket->get_object(
            rgw_obj_key(encoded_obj_str, s->info.args.get("versionId")));
      } else {
    ...
    if (s->bucket) {
      s->object = s->bucket->get_object(
          rgw_obj_key(req_name, s->info.args.get("versionId")));
    } else {
      s->object = driver->get_object(
          rgw_obj_key(req_name, s->info.args.get("versionId")));
    }
...
int RGWHandler_REST_S3::postauth_init(optional_yield y) {
  ...
  int ret = rgw_parse_url_bucket(t->url_bucket, auth_tenant, s->bucket_tenant,
                                 s->bucket_name);
  if (ret) {
    return ret;
  }

  ldpp_dout(s, 10) << "s->object=" << s->object << " s->bucket="
                   << rgw_make_bucket_entry_name(s->bucket_tenant,
                                                 s->bucket_name)
                   << dendl;
  ...
  if (!t->src_bucket.empty()) {
    ret = rgw_parse_url_bucket(t->src_bucket, auth_tenant, s->src_tenant_name,
                               s->src_bucket_name);
    if (ret) {
      return ret;
    }
...
int RGWHandler_REST_S3::init(rgw::sal::Driver *driver, req_state *s,
  ...
  if (copy_source && (!s->info.env->get("HTTP_X_AMZ_COPY_SOURCE_RANGE")) &&
    ...
    ret = RGWCopyObj::parse_copy_location(copy_source, s->init_state.src_bucket,
                                          key, s);
    if (!ret) {
...
int RGWHandler_REST_S3::authorize(const DoutPrefixProvider *dpp,
  ...
  if (s->info.args.exists("Action") &&
      s->info.args.get("Action") == "AssumeRoleWithWebIdentity") {
    return RGW_Auth_STS::authorize(dpp, driver, auth_registry, s, y);
  }
  return RGW_Auth_S3::authorize(dpp, driver, auth_registry, s, y);
}

enum class AwsVersion { UNKNOWN, V2, V4 };

enum class AwsRoute { UNKNOWN, QUERY_STRING, HEADERS };
...
int RGW_Auth_S3::authorize(const DoutPrefixProvider *dpp,
  ...
  if (!driver->ctx()->_conf->rgw_s3_auth_use_rados &&
    ...
    ldpp_dout(dpp, 0) << "WARNING: no authorization backend enabled! Users "
...
int RGWHandler_Auth_S3::init(rgw::sal::Driver *driver, req_state *state,
                             rgw::io::BasicClient *cio) {
  int ret = RGWHandler_REST_S3::init_from_header(driver, state, RGWFormat::JSON,
                                                 true);
  if (ret < 0)
    return ret;
...
namespace {
// utility classes and functions for handling parameters with the following
// format: Attributes.entry.{N}.{key|value}={VALUE} N - any unsigned number
...
void update_attribute_map(const std::string &input, AttributeMap &map) {
  ...
  if (pos != std::string::npos) {
    ...
    const auto key_or_value_rhs = url_decode(
        key_or_value.substr(pos + 1, key_or_value.size() - 1), in_query);
    const auto map_it = map.find(idx);
    if (map_it == map.end()) {
      // new entry
      map.emplace(
          std::make_pair(idx, Attribute(key_or_value_lhs, key_or_value_rhs)));
    } else {
      // existing entry
...
void parse_post_action(const std::string &post_body, req_state *s) {
  if (post_body.size() > 0) {
    if (post_body.find("Action") != string::npos) {
      const boost::char_separator<char> sep("&");
      const boost::tokenizer<boost::char_separator<char>> tokens(post_body,
                                                                 sep);
      AttributeMap map;
...
RGWHandler_REST *
RGWRESTMgr_S3::get_handler(rgw::sal::Driver *driver, req_state *const s,
                           const rgw::auth::StrategyRegistry &auth_registry,
                           const std::string &frontend_prefix) {
  bool is_s3website = enable_s3website && (s->prot_flags & RGW_REST_WEBSITE);
  int ret = RGWHandler_REST_S3::init_from_header(
      driver, s, is_s3website ? RGWFormat::HTML : RGWFormat::XML, true);
  if (ret < 0) {
    return nullptr;
  }
...
bool RGWHandler_REST_S3Website::web_dir() const {
  ...
  std::unique_ptr<rgw::sal::Object> obj =
      s->bucket->get_object(rgw_obj_key(subdir_name));
...
int RGWHandler_REST_S3Website::retarget(RGWOp *op, RGWOp **new_op,
  ...
  *new_op = op;
  ...
  bool get_res = s->bucket->get_info().website_conf.get_effective_key(
      key_name, &new_obj.name, web_dir());
  if (!get_res) {
    s->err.message =
  ...
  bool should_redirect = s->bucket->get_info().website_conf.should_redirect(
      new_obj.name, 0, &rrule);
...
RGWOp *RGWHandler_REST_S3Website::op_head() { return get_obj_op(false); }

int RGWHandler_REST_S3Website::serve_errordoc(const DoutPrefixProvider *dpp,
                                              int http_ret,
                                              const string &errordoc_key,
                                              optional_yield y) {
  int ret = 0;
  s->formatter->reset(); /* Try to throw it all away */

  std::shared_ptr<RGWGetObj_ObjStore_S3Website> getop(
      static_cast<RGWGetObj_ObjStore_S3Website *>(op_get()));
  if (getop.get() == NULL) {
  ...
  if (ret < 0) {
    ldpp_dout(s, 20) << "serve_errordoc failed, init_permissions ret=" << ret
                     << dendl;
    return -1; // Trigger double error handler
  }
  ...
  if (ret < 0) {
    ldpp_dout(s, 20) << "serve_errordoc failed, read_permissions ret=" << ret
                     << dendl;
    return -1; // Trigger double error handler
  }
  ...
  if (ret < 0) {
    ldpp_dout(s, 20) << "serve_errordoc failed, init_processing ret=" << ret
                     << dendl;
    return -1; // Trigger double error handler
  }
  ...
  if (ret < 0) {
    ldpp_dout(s, 20) << "serve_errordoc failed, verify_op_mask ret=" << ret
                     << dendl;
    return -1; // Trigger double error handler
  }
  ...
  if (ret < 0) {
    ldpp_dout(s, 20) << "serve_errordoc failed, verify_permission ret=" << ret
                     << dendl;
    return -1; // Trigger double error handler
  }
  ...
  if (ret < 0) {
    ldpp_dout(s, 20) << "serve_errordoc failed, verify_params ret=" << ret
                     << dendl;
    return -1; // Trigger double error handler
  }
...
int RGWHandler_REST_S3Website::error_handler(int err_no, string *error_content,
                                             optional_yield y) {
  int new_err_no = -1;
  rgw_http_errors::const_iterator r =
      rgw_http_s3_errors.find(err_no > 0 ? err_no : -err_no);
  int http_error_code = -1;

  if (r != rgw_http_s3_errors.end()) {
    http_error_code = r->second.first;
  }
  ldpp_dout(s, 10) << "RGWHandler_REST_S3Website::error_handler err_no="
                   << err_no << " http_ret=" << http_error_code << dendl;

  RGWBWRoutingRule rrule;
  bool have_bucket = !rgw::sal::Bucket::empty(s->bucket.get());
  bool should_redirect = false;
  if (have_bucket) {
    should_redirect = s->bucket->get_info().website_conf.should_redirect(
        original_object_name, http_error_code, &rrule);
  }
  ...
  if (should_redirect) {
    ...
    const string &protocol =
        (rgw_transport_is_secure(s->cct, *s->info.env) ? "https" : "http");
    int redirect_code = 0;
    rrule.apply_rule(protocol, hostname, original_object_name, &s->redirect,
                     &redirect_code);
    // Apply a custom HTTP response code
    if (redirect_code > 0)
      s->err.http_ret = redirect_code; // Apply a custom HTTP response code
    ...
    // Do nothing here, this redirect will be handled in abort_early's
    // ERR_WEBSITE_REDIRECT block Do NOT fire the ErrorDoc handler
  } else if (have_bucket &&
             !s->bucket->get_info().website_conf.error_doc.empty()) {
    /* This serves an entire page!
       On success, it will return zero, and no further content should be sent to
       the socket On failure, we need the double-error handler
     */
    new_err_no = RGWHandler_REST_S3Website::serve_errordoc(
        s, http_error_code, s->bucket->get_info().website_conf.error_doc, y);
    if (new_err_no != -1) {
      err_no = new_err_no;
    }
  } else {
...
boost::optional<std::string> AWSGeneralAbstractor::get_v4_canonical_headers(
    const req_info &info, const std::string_view &signedheaders,
    const bool using_qs) const {
  return rgw::auth::s3::get_v4_canonical_headers(info, signedheaders, using_qs,
                                                 false);
}

AWSSignerV4::prepare_result_t
AWSSignerV4::prepare(const DoutPrefixProvider *dpp,
                     const std::string &access_key_id, const string &region,
                     const string &service, const req_info &info,
                     const bufferlist *opt_content, bool s3_op) {
  ...
  std::string date =
      ceph::to_iso_8601_no_separators(timestamp, ceph::iso_8601_format::YMDhms);
  ...
  ldpp_dout(dpp, 0) << "AWSSignerV4::prepare: canonical_uri = " << canonical_uri
                    << dendl;
  ...
  ldpp_dout(dpp, 0) << "AWSSignerV4::prepare: canonical_qs = " << canonical_qs
                    << dendl;
  ldpp_dout(dpp, 0) << "AWSSignerV4::prepare: is_non_s3_op = " << is_non_s3_op
                    << dendl;

  auto cct = dpp->get_cct();

  // Log components before creating canonical request hash
  ldpp_dout(dpp, 0) << "AWSSignerV4::prepare: Creating canonical request with:"
                    << dendl;
  ldpp_dout(dpp, 0) << "  method = " << info.method << dendl;
  ldpp_dout(dpp, 0) << "  canonical_uri = " << canonical_uri << dendl;
  ldpp_dout(dpp, 0) << "  canonical_qs = " << canonical_qs << dendl;
  ldpp_dout(dpp, 0) << "  canonical_headers = " << canonical_headers << dendl;
  ldpp_dout(dpp, 0) << "  signed_hdrs = " << signed_hdrs << dendl;
  ldpp_dout(dpp, 0) << "  exp_payload_hash = "
                    << (exp_payload_hash ? exp_payload_hash : "NULL") << dendl;

  /* Craft canonical request. */
  auto canonical_req_hash = rgw::auth::s3::get_v4_canon_req_hash(
      cct, info.method, std::move(canonical_uri), std::move(canonical_qs),
      std::move(canonical_headers), signed_hdrs, exp_payload_hash, dpp);

  ldpp_dout(dpp, 0) << "AWSSignerV4::prepare: canonical_req_hash = "
                    << canonical_req_hash << dendl;

  auto string_to_sign = rgw::auth::s3::get_v4_string_to_sign(
      cct, AWS4_HMAC_SHA256_STR, date, credential_scope,
      std::move(canonical_req_hash), dpp);

  ldpp_dout(dpp, 0) << "AWSSignerV4::prepare: string_to_sign = "
...
gen_v4_signature(const DoutPrefixProvider *dpp,
  ...
  auto signature = rgw::auth::s3::get_v4_signature(
      sig_info.scope, dpp->get_cct(), secret_key, sig_info.string_to_sign, dpp);
  ...
  string auth_header =
      string("AWS4-HMAC-SHA256 Credential=").append(sig_info.access_key_id) +
      "/";
...
AWSGeneralAbstractor::get_auth_data_v4(const req_state *const s,
  ...
  int ret = rgw::auth::s3::parse_v4_credentials(
      s->info, access_key_id, credential_scope, signed_hdrs, client_signature,
      date, session_token, using_qs, s);
  if (ret < 0) {
    throw ret;
  }
  ...
  /* Craft canonical query string. std::moving later so non-const here. */
  auto canonical_qs = rgw::auth::s3::get_v4_canonical_qs(s->info, using_qs, s);
  ...
  /* Craft canonical request. */
  auto canonical_req_hash = rgw::auth::s3::get_v4_canon_req_hash(
      s->cct, std::move(canonical_method), std::move(canonical_uri),
      std::move(canonical_qs), std::move(*canonical_headers), signed_hdrs,
      exp_payload_hash, s);

  auto string_to_sign = rgw::auth::s3::get_v4_string_to_sign(
      s->cct, AWS4_HMAC_SHA256_STR, date, credential_scope,
      std::move(canonical_req_hash), s);

  const auto sig_factory = std::bind(
      rgw::auth::s3::get_v4_signature, credential_scope, std::placeholders::_1,
      std::placeholders::_2, std::placeholders::_3, s);
  ...
  if (is_v4_payload_empty(s) && !is_v4_payload_unsigned(exp_payload_hash) &&
  ...
  if (traditional_v4_unsigned || (v4_unsigned && !checksum_trailer) ||
      is_v4_payload_empty(s) || is_non_s3_op) {
    ldpp_dout(s, 10) << __func__
                     << ": UNSIGNED-PAYLOAD or other v4 no-completer case"
                     << dendl;
    return {access_key_id, client_signature,
            session_token, std::move(string_to_sign),
            sig_factory,   null_completer_factory};
  } else {
    ...
    if (!is_v4_payload_streamed(exp_payload_hash)) {
      ...
      case RGW_OP_INIT_MULTIPART: // in case that Init Multipart uses CHUNK
                                  // encoding
      ...
      case RGW_OP_GET_OBJ: // s3select its post-method(payload contain the
                           // query) , the request is get-object
        break;
      default:
        ldpp_dout(s, 10) << "ERROR: AWS4 completion for operation: "
                         << s->op_type << ", NOT IMPLEMENTED" << dendl;
        throw -ERR_NOT_IMPLEMENTED;
      }

      const auto cmpl_factory =
          std::bind(AWSv4ComplSingle::create, s, std::placeholders::_1);
      return {access_key_id, client_signature,
              session_token, std::move(string_to_sign),
              sig_factory,   cmpl_factory};
    } else {
      ...
      case RGW_OP_PUT_OBJ:
        break;
      default:
        ldpp_dout(s, 10) << "ERROR: AWS4 completion for this operation NOT "
      ...
      const auto cmpl_factory =
          std::bind(AWSv4ComplMulti::create, s, date, credential_scope,
                    client_signature, flags, std::placeholders::_1);
      return {access_key_id, client_signature,
              session_token, std::move(string_to_sign),
              sig_factory,   cmpl_factory};
    }
...
AWSGeneralAbstractor::get_auth_data_v2(const req_state *const s) const {
  ...
  return {std::move(access_key_id),        std::move(signature),
          std::move(session_token),        std::move(string_to_sign),
          rgw::auth::s3::get_v2_signature, null_completer_factory};
}

AWSEngine::VersionAbstractor::auth_data_t
AWSBrowserUploadAbstractor::get_auth_data_v2(const req_state *const s) const {
  return {s->auth.s3_postobj_creds.access_key,
          s->auth.s3_postobj_creds.signature,
          s->auth.s3_postobj_creds.x_amz_security_token,
...
AWSEngine::result_t AWSEngine::authenticate(const DoutPrefixProvider *dpp,
  ...
  if (auth_data.access_key_id.empty() || auth_data.client_signature.empty()) {
    ...
    return authenticate(dpp, auth_data.access_key_id,
                        auth_data.client_signature, auth_data.session_token,
                        auth_data.string_to_sign, auth_data.signature_factory,
                        auth_data.completer_factory, s, y);
  }
}
...
void rgw::auth::s3::LDAPEngine::init(CephContext *const cct) {
  if (!cct->_conf->rgw_s3_auth_use_ldap || cct->_conf->rgw_ldap_uri.empty()) {
    return;
  }
...
rgw::auth::RemoteApplier::AuthInfo rgw::auth::s3::LDAPEngine::get_creds_info(
    const rgw::RGWToken &token) const noexcept {
...
rgw::auth::Engine::result_t rgw::auth::s3::LDAPEngine::authenticate(
    const DoutPrefixProvider *dpp, const std::string_view &access_key_id,
    const std::string_view &signature, const std::string_view &session_token,
    const string_to_sign_t &string_to_sign, const signature_factory_t &,
    const completer_factory_t &completer_factory, const req_state *const s,
    optional_yield y) const {
  ...
  if (rgw_get_user_info_by_uid(driver, user_info.user_id, user_info) >= 0) {
    if (user_info.type != TYPE_LDAP) {
      ldpp_dout(dpp, 10) << "ERROR: User id of type: " << user_info.type << " is
  already present" << dendl; return nullptr;
    }
  }*/
...
/* LocalEngine */
rgw::auth::Engine::result_t rgw::auth::s3::LocalEngine::authenticate(
    const DoutPrefixProvider *dpp, const std::string_view &_access_key_id,
    const std::string_view &signature, const std::string_view &session_token,
  ...
  // Simplified server-side logging
  ldpp_dout(dpp, 0) << "[SERVER] string_to_sign:\n" 
                    << rgw::crypt_sanitize::log_content{string_to_sign} << dendl;
  ...
  ldpp_dout(dpp, 0) << "[SERVER] server_signature=" << server_signature << dendl;
  ldpp_dout(dpp, 0) << "[SERVER] client_signature=" << signature << dendl;
  if (compare != 0) {
    ldpp_dout(dpp, 0) << "[SERVER] SIGNATURE MISMATCH!" << dendl;
  }
...
rgw::auth::RemoteApplier::AuthInfo rgw::auth::s3::STSEngine::get_creds_info(
    const STS::SessionToken &token) const noexcept {
  using acct_privilege_t = rgw::auth::RemoteApplier::AuthInfo::acct_privilege_t;
  ...
  return rgw::auth::RemoteApplier::AuthInfo{
      ...
      token.acct_name,
      token.perm_mask,
      (token.is_admin) ? acct_privilege_t::IS_ADMIN_ACCT
                       : acct_privilege_t::IS_PLAIN_ACCT,
      token.access_key_id,
      rgw::auth::RemoteApplier::AuthInfo::NO_SUBUSER,
      token.acct_type};
...
int rgw::auth::s3::STSEngine::get_session_token(
    const DoutPrefixProvider *dpp, const std::string_view &session_token,
    STS::SessionToken &token) const {
  string decodedSessionToken;
  try {
    decodedSessionToken = rgw::from_base64(session_token);
  } catch (...) {
    ldpp_dout(dpp, 0) << "ERROR: Invalid session token, not base64 encoded."
                      << dendl;
    return -EINVAL;
  }
  ...
  if (ret < 0) {
    ...
    try {
      ...
      ldpp_dout(dpp, 0) << "ERROR: decode SessionToken failed: " << error
                        << dendl;
      return -EINVAL;
    }
  }
...
rgw::auth::Engine::result_t rgw::auth::s3::STSEngine::authenticate(
    const DoutPrefixProvider *dpp, const std::string_view &_access_key_id,
    const std::string_view &signature, const std::string_view &session_token,
    const string_to_sign_t &string_to_sign,
    const signature_factory_t &signature_factory,
  ...
  if (token.access_key_id != _access_key_id) {
    ...
    // only allow system users as this could be a forwarded request from
    // secondary
    if (user->get_info().system && driver->is_meta_master()) {
      const auto iter = user->get_info().access_keys.find(access_key_id);
      if (iter == std::end(user->get_info().access_keys)) {
        ldpp_dout(dpp, 0) << "ERROR: access key not encoded in user info"
                          << dendl;
  ...
  if (!token.expiration.empty()) {
    ...
    if (!expiration.empty()) {
      boost::optional<real_clock::time_point> exp =
          ceph::from_iso_8601(expiration, false);
      if (exp) {
        ...
        ldpp_dout(dpp, 0) << "ERROR: Invalid expiration: " << expiration
                          << dendl;
        return result_t::reject(-EPERM);
      }
    }
  ...
  if (!token.roleId.empty()) {
    ...
    if (!account_id.empty()) {
      ...
      int ret = driver->load_account_by_id(dpp, y, account_id, *r.account,
                                           attrs, objv);
      if (ret < 0) {
        ldpp_dout(dpp, 1) << "ERROR: failed to load account " << account_id
                          << " for role " << r.name << ": " << cpp_strerror(ret)
                          << dendl;
        return result_t::deny(-EPERM);
      }
    }
  ...
  if (token.acct_type == TYPE_KEYSTONE || token.acct_type == TYPE_LDAP) {
    ...
    t_attrs.user_id =
        std::move(token.user); // This is mostly needed to assign the owner of a
                               // bucket during its creation
    ...
    auto apl = role_apl_factory->create_apl_role(
        cct, s, std::move(r), std::move(t_attrs), is_system_request);
    return result_t::grant(std::move(apl),
                           completer_factory(token.secret_access_key));
  } else { // This is for all local users of type TYPE_RGW|ROOT|NONE
    if (token.user.empty()) {
      ldpp_dout(dpp, 5) << "ERROR: got session token with empty user id"
                        << dendl;
      return result_t::reject(-EPERM);
    }
    // load user info
    ...
    if (ret < 0) {
      ldpp_dout(dpp, 5) << "ERROR: failed reading user info: uid=" << token.user
                        << dendl;
      return result_t::reject(-EPERM);
    }
...
bool rgw::auth::s3::S3AnonymousEngine::is_applicable(
    const req_state *s) const noexcept {
  AwsVersion version;
  AwsRoute route;
  std::tie(version, route) = discover_aws_flavour(s->info);
